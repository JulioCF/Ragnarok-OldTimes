...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv)
{
	...
	if( sc && sc->count )
	{
		...
		if( /*(*/ sc->data[SC_PNEUMA] /*|| group->unit_id == UNT_ICEWALL )*/ && (flag&(BF_MAGIC|BF_LONG)) == BF_LONG )//beta
		{
			d->dmg_lv = ATK_BLOCK;
			return 0;
		}
...
static struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	{	//Hit/Flee calculation
		...
		{	//Hit skill modifiers
			...
			case SM_BASH:
			case MS_BASH:
				hitrate += hitrate * skill_lv / 20;
				break;
			case MS_MAGNUM:
			case SM_MAGNUM:
				hitrate += hitrate * skill_lv / 10;
				break;
			case KN_AUTOCOUNTER:
			case PA_SHIELDCHAIN:
			...
			switch( skill_num )
			{
				...
				case RG_BACKSTAP:
					if(sd && sd->status.weapon == W_BOW && battle_config.backstab_bow_penalty)
						skillratio += 20*(5+skill_lv);
					else
						skillratio += 40*(5+skill_lv);
					break;
				case RG_RAID:
					skillratio += 40*skill_lv;
					break;
				...
				case MO_TRIPLEATTACK:
					...
					break;
				case MO_CHAINCOMBO:
					skillratio += 50*(1+skill_lv);
					break;
				case MO_COMBOFINISH:
					skillratio += 4*(35+15*skill_lv);
					break;
				case BA_MUSICALSTRIKE:
				case DC_THROWARROW:
					skillratio += 25*(1+skill_lv);
					break;
				case CH_TIGERFIST:
					skillratio += 20*(5*skill_lv-3);
					break;
				case CH_CHAINCRUSH:
					skillratio += 100*(3+skill_lv);
					break;
				case CH_PALMSTRIKE:
					skillratio += 100*(1+skill_lv);
					break;
				case LK_HEADCRUSH:
					skillratio += 40*skill_lv;
					break;
				...
				case ASC_METEORASSAULT:
					skillratio += 20*(2*skill_lv-3);
					break;
				case SN_SHARPSHOOTING:
				case MA_SHARPSHOOTING:
					skillratio += 50*(2+skill_lv);
					break;
				case CG_ARROWVULCAN:
					skillratio += 100*(1+skill_lv);
					break;
				case AS_SPLASHER:
					skillratio += 50*(8+skill_lv);
					if(sd)
				...
				case WS_CARTTERMINATION:
					...
					break;
				case TK_DOWNKICK:
					skillratio += 20*(3 + skill_lv);
					break;
				case TK_STORMKICK:
					skillratio += 20*(3 + skill_lv);
					break;
				case TK_TURNKICK:
					skillratio += 30*(3 + skill_lv);
					break;
				case TK_COUNTER:
					skillratio += 30*(3 + skill_lv);
					break;
				case TK_JUMPKICK:
					skillratio += 10*(skill_lv - 7);
	...
	switch( src->type )
  	{	//Checks on actual src type
		...
		case BL_SKILL:
		{
			...
			if( battle_config.vs_traps_bctall && (inf2 = skill_get_inf2(su->group->skill_id))&INF2_TRAP && map_flag_vs(src->m) )
