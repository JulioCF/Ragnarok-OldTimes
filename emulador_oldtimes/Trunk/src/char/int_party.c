...
int inter_party_sql_init(void)
{
	...
	if (!party_pt) {
		ShowFatalError("inter_party_sql_init: memÛria insuficiente!\n");
		exit(EXIT_FAILURE);
	}
...
static void mapif_party_noinfo(int fd, int party_id, int char_id)
{
	...
	WFIFOSET(fd,12);
	ShowWarning("int_party: informaÁ„o n„o encontrada (party_id=%d char_id=%d)\n", party_id, char_id);
}
...
// Wheter additional party members
...
// Party setting's change notification
int mapif_party_optionchanged(int fd,struct party *p,int account_id,int flag)
{
...
// Party setting's change request
...
// Communication from the map-server
// ÅYou can only analyze a packet
// Data packet length is set in inter.c
// ÅDo not go and check the packet length, RFIFOSKIP is done by the caller
// ÅFalse must return 0. True must return 1.
...
int inter_party_CharOnline(int char_id, int party_id)
{
	...
	p = inter_party_fromsql(party_id);
	if(!p) {
		ShowError("Grupo %d da personagem %d n„o foi encotrado!\n", party_id, char_id);
		return 0;
	}
