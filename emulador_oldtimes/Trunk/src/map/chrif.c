...
void chrif_checkdefaultlogin(void)
{
	if (strcmp(userid, "s1")==0 && strcmp(passwd, "p1")==0) {
		ShowWarning("AVISO DE SEGURAN«A - O uso do login/senha padrıes s1/p1 n„o È recomendado.\n");
		ShowNotice("Edite a tabela 'login' para criar uma certa configuraÁ„o inter-server login/senha.\n");
...
int chrif_setip(const char* ip)
{
	...
	ShowInfo("EndereÁo IP do char-server: '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(char_ip, ip_str));
	return 1;
}
...
int chrif_changemapserverack(int account_id, int login_id1, int login_id2, int char_id, short map_index, short x, short y, uint32 ip, uint16 port)
{
	...
	if (!login_id1) {
		ShowError("MudanÁa do map-server falhou.\n");
...
int chrif_connectack(int fd)
{
	...
	if (RFIFOB(fd,2)) {
		ShowFatalError("Conex„o ao char-server falhou %d.\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}

	ShowStatus(" xito na conex„o ao char-server (Conex„o: '"CL_WHITE"%d"CL_RESET"').\n",fd);
	chrif_state = 1;
	chrif_connected = 1;
...
{
	ShowStatus("Map-server est· online!\n");
	chrif_state = 2;
	chrif_check_shutdown();
...
int chrif_accountban(int fd)
{
	...
	{ 
		...
		case 6: clif_displaymessage(sd->fd, "Seu EXE do jogo n„o est· na ˙ltima vers„o."); break;
		case 7: clif_displaymessage(sd->fd, "Sua conta foi proibida de fazer login."); break;
		case 8: clif_displaymessage(sd->fd, "Servidor est· lotado."); break;
...
int chrif_load_scdata(int fd)
{
	...
	sd = map_id2sd(aid);
	if (!sd) {
		ShowError("chrif_load_scdata: Jogador de AID %d n„o encontrado!\n", aid);
		return -1;
	}
...
void chrif_on_disconnect(void)
{
	if( chrif_connected != 1 )
		ShowWarning("Conex„o ao char-server perdida.\n\n");
	chrif_connected = 0;
...
int chrif_parse(int fd)
{
	...
	if (fd != char_fd)
	{
		ShowDebug("chrif_parse: Desconectando sess„o inv·lida #%d (n„o È o char-server)\n", fd);
		do_close(fd);
		return 0;
	}
	...
	while (RFIFOREST(fd) >= 2)
	{
		...
		if (cmd < 0x2af8 || cmd >= 0x2af8 + ARRAYLENGTH(packet_len_table) || packet_len_table[cmd-0x2af8] == 0)
		{
			...
			ShowWarning("chrif_parse: session #%d, intif_parse falhou (comando 0x%.4x n„o reconhecido).\n", fd, cmd);
			set_eof(fd);
			return 0;
		}
		...
		default:
			ShowError("chrif_parse : pacote desconhecido (sess„o #%d): 0x%x. Desconectando.\n", fd, cmd);
			set_eof(fd);
			return 0;
		}
