...
{	//set SO_LINGER option (from Freya)
	...
	if (setsockopt(fd, SOL_SOCKET, SO_LINGER, (char*)&opt, sizeof(opt)))
		ShowWarning("setsocketopts: impossivel setar SO_LINGER modo de conexao %d!\n",fd);
}
#endif
	...
	if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, (void *)&buff, &buff_size) == 0)
	{
		if (buff < wfifo_size) //We are not going to complain if we get more, aight? [Skotlex]
			ShowError("setsocketopts: O pedido de envio de buffer falhou(pedido %d bytes buffer, envio buffer de tamanho %d)\n", wfifo_size, buff);
	}
	else
	...
	{
		if (buff < rfifo_size)
			ShowError("setsocketopts: O pedido de recebimento de buffer falhou(pedido %d bytes buffer, recebido buffer de tamanho %d)\n", rfifo_size, buff);
	}
	else
...
int make_listen_port(int port)
{
	...
	if (fd == INVALID_SOCKET) {
		ShowError("socket() a criacao failed (code %d)!\n", fd, WSAGetLastError());
		exit(1);
	}
#else
	if (fd == -1) {
...
#ifdef __WIN32
	if( result == SOCKET_ERROR ) {
		ShowError("a escuta falhou (socket %d, code %d)!\n", fd, WSAGetLastError());
		exit(1);
	}
#else
	if( result != 0 ) { /* error */
	...
	{ //Crazy error that can happen in Windows? (info from Freya)
		ShowFatalError("listen() retorno invalido fd %d!\n",fd);
		exit(1);
	}
...
int make_listen_bind(long ip,int port)
{
...
#ifdef __WIN32
	if (fd == INVALID_SOCKET) {
		ShowError("socket() a criacao falhou (code %d)!\n", fd, WSAGetLastError());
		exit(1);
	}
#else
	if (fd == -1) {
	...
	if ( fd < 0 || fd > FD_SETSIZE )
	{ //Crazy error that can happen in Windows? (info from Freya)
		ShowFatalError("a escuta() retornou invalida fd %d!\n",fd);
		exit(1);
	}
	...
	ShowStatus("escutando porta %d.%d.%d.%d:%i\n",
...
int console_recieve(int i) {
	...
	n = read(0, buf , 64);
	if ( n < 0 )
		ShowError("O console possui um erro de input\n");
	else
	{
		ShowNotice (" Desculpe o console esta em 'nao-funcional'.\n");
...
int start_console(void) {
	...
	ShowNotice("O Console esta em modo nao funcional.\n");
	return 0;
...
int make_connection(long ip,int port)
{
	...
	ShowStatus("conectando to %d.%d.%d.%d:%i\n",
		(ip)&0xFF,(ip>>8)&0xFF,(ip>>16)&0xFF,(ip>>24)&0xFF,port);

	result = connect(fd, (struct sockaddr *)(&server_address), sizeof(struct sockaddr_in));
#ifdef __WIN32
	if( result == SOCKET_ERROR ) {
		ShowError("conexao falhou (socket %d, code %d)!\n", fd, WSAGetLastError());
		do_close(fd);
		return -1;
	}
#else
...
int WFIFOSET(int fd,int len)
{
	...
	{	// actually there was a buffer overflow already
		...
		ShowMessage("socket: Buffer baixo. Conexao %d (%d.%d.%d.%d) foi escrito %d bytes (%d habilitado)\n", fd,
...
int do_sendrecv(int next)
{
	...
	for (i = 1; i < fd_max; i++){ //Session 0 is never a valid session, so it's best to skip it. [Skotlex]
		if(!session[i]) {
			if (FD_ISSET(i, &readfds)){
				ShowDebug("limpeza forcada fds %d\n", i);
				FD_CLR(i, &readfds);
				//FD_CLR(i, &rfd);
				//FD_CLR(i, &efd);
			}
	...
	if (ret == SOCKET_ERROR) {
		...
		ShowError("do_sendrecv: erro de selecao (code %d)\n", WSAGetLastError());
#else
	if (ret < 0) {
		perror("do_sendrecv");
		...
		for(i = 1; i < fd_max; i++) { //Session 0 is not parsed, it's a 'vacuum' for disconnected sessions. [Skotlex]
			if (!session[i]) {
				...
				if (FD_ISSET(i, &readfds)) {
					FD_CLR(i, &readfds);
					ShowDebug("Socket %d foi setado (leia fifos) fora da sessao, removido.\n", i);
				}
#endif
				continue;
			}
			...
			if (select(i + 1, &rfd, &wfd, &efd, &timeout) >= 0 && !FD_ISSET(i, &efd)) {
				...
				ShowDebug("do_sendrecv: Sessao #%d causou erro de selecao(), desconectando.\n", i);
				set_eof(i); // set eof
		...
		for (i = 1; i < fd_max; i++){
			...
			if(FD_ISSET(i,&efd)){
				//ShowMessage("error:%d\n",i);
				ShowDebug("do_sendrecv: Erro de conexao na sessao %d.\n", i);
				set_eof(i);
				continue;
			}
...
int do_parsepacket(void)
{
	...
	for(i = 1; i < fd_max; i++){
		...
		if ((sd->rdata_tick != 0) && DIFF_TICK(last_tick,sd->rdata_tick) > stall_time) {
			ShowInfo ("Sessao #%d em tempo esgotado\n", i);
			sd->eof = 1;
		}
...
// ê⁄ë±Ç≈Ç´ÇÈÇ©Ç«Ç§Ç©ÇÃämîF
//   false : ê⁄ë±OK
//   true  : ê⁄ë±NG
static int connect_check(unsigned int ip) {
	int result = connect_check_(ip);
	if(access_debug) {
		ShowMessage("connect_check: Conexao para %d.%d.%d.%d %s\n",
			CONVIP(ip),result ? "aceito." : "negado!");
	}
	return result;
}
...
static int connect_check_(unsigned int ip) {
	...
	// allow , deny ÉäÉXÉgÇ…ì¸Ç¡ÇƒÇ¢ÇÈÇ©ämîF
	for(i = 0;i < access_allownum; i++) {
		if((ip & access_allow[i].mask) == (access_allow[i].ip & access_allow[i].mask)) {
			if(access_debug) {
				ShowMessage("connect_check: Encontrado lista de aceitacao:%d.%d.%d.%d IP:%d.%d.%d.%d Mask:%d.%d.%d.%d\n",
					CONVIP(ip),
	...
	for(i = 0;i < access_denynum; i++) {
		if((ip & access_deny[i].mask) == (access_deny[i].ip & access_deny[i].mask)) {
			if(access_debug) {
				ShowMessage("connect_check: Encontrado lista de negacao:%d.%d.%d.%d IP:%d.%d.%d.%d Mask:%d.%d.%d.%d\n",
					CONVIP(ip),
	...
	// ÉRÉlÉNÉgèoóàÇÈÇ©Ç«Ç§Ç©ämîF
	// connect_ok
	//   0 : ñ≥èåèÇ…ãëî€
	//   1 : ìcë„ñCÉ`ÉFÉbÉNÇÃåãâ éüëÊ
	//   2 : ñ≥èåèÇ…ãñâ¬
	switch(access_order) {
	case ACO_DENY_ALLOW:
	default:
	...
	while(hist) {
		if(ip == hist->ip) {
			// ìØÇ∂IPî≠å©
			if(hist->status) {
				// ban ÉtÉâÉOÇ™óßÇ¡ÇƒÇÈ
				return (connect_ok == 2 ? 1 : 0);
			} else if(DIFF_TICK(gettick(),hist->tick) < ddos_interval) {
				// ddos_intervalïbà»ì‡Ç…ÉäÉNÉGÉXÉgóLÇË
				hist->tick = gettick();
				if(hist->count++ >= ddos_count) {
					// ddos çUåÇÇåüèo
					hist->status = 1;
					ShowWarning("connect_check: Ataque de DDOS para %d.%d.%d.%d!\n",
						CONVIP(ip));
					return (connect_ok == 2 ? 1 : 0);
				} else {
					return connect_ok;
				...
				// ddos_intervalïbà»ì‡Ç…ÉäÉNÉGÉXÉgñ≥Ç¢ÇÃÇ≈É^ÉCÉ}Å[ÉNÉäÉA
				hist->tick  = gettick();
				hist->count = 0;
				return connect_ok;
			}
...
static int connect_check_clear(int tid,unsigned int tick,int id,int data) {
	...
	if(access_debug) {
		ShowMessage("connect_check_clear: Limpado %d de %d para lista de IP.\n", clear, list);
	}
	return list;
}
...
int access_ipmask(const char *str,struct _access_control* acc)
{
	...
	if( !strcmp(str,"all") ) {
		...
		if( sscanf(str,"%d.%d.%d.%d%n",&a0,&a1,&a2,&a3,&i)!=4 || i==0) {
			ShowError("access_ipmask: Formato desconhecido %s!\n",str);
			return 0;
		}
	...
	if(access_debug) {
		ShowMessage("access_ipmask: Carregado IP:%d.%d.%d.%d mask:%d.%d.%d.%d\n",
...
int RFIFOSKIP(int fd,int len)
{
	...
	if (s->rdata_size-s->rdata_pos-len<0) {
		...
		ShowError("muitos avancos (%d) agora avancado: %d (FD: %d)\n", len, RFIFOREST(fd), fd);
		len = RFIFOREST(fd);
	}
...
void socket_final (void)
{
	...
	// session[0] ÇÃÉ_É~Å[ÉfÅ[É^ÇçÌèú
...
void socket_init (void)
{
	...
	if ( WSAStartup(version_wanted, &wsaData) != 0 ) {
		ShowFatalError("SYSERR: WinSock nao definido!\n");
		exit(1);
	}

	if(gethostname(fullhost, sizeof(fullhost)) == SOCKET_ERROR) {
		ShowError("Ugg.. sem hostname definido!\n");
		return;
	}
	...
	if (hent == NULL) {
		ShowError("Nao pode resolver um proprio hostname a um endereco de IP");
		return;
	}
...
#ifndef MINICORE
	// Ç∆ÇËÇ†Ç¶Ç∏ÇTï™Ç≤Ç∆Ç…ïsóvÇ»ÉfÅ[É^ÇçÌèúÇ∑ÇÈ
