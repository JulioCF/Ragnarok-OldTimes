...
int inter_accreg_init() {
	...
	while(fgets(line, sizeof(line)-1, fp)){
		...
		if (reg == NULL) {
			ShowFatalError("inter: accreg: despejo de memoria!\n");
			exit(0);
		}
		if (inter_accreg_fromstr(line, reg) == 0 && reg->account_id > 0) {
			numdb_insert(accreg_db, reg->account_id, reg);
		} else {
			ShowError("inter: accreg: data corrompida [%s] linha %d\n", accreg_txt, c);
			aFree(reg);
		}
		c++;
	}
...
int inter_accreg_save() {
	...
	if ((fp = lock_fopen(accreg_txt,&lock)) == NULL) {
		ShowError("int_accreg: impossivel ler [%s] !!! data corrompida !!!\n", accreg_txt);
		return 1;
	}
...
int mapif_parse_WisRequest(int fd) {
	...
	if (RFIFOW(fd,2)-52 >= sizeof(wd->msg)) {
		ShowWarning("inter: Esta mensagem esta muito longa.\n");
		return 0;
	...
	// Character exists. So, ask all map-servers
		...
		if (strcmp((char*)RFIFOP(fd,4),name) == 0) {
			...
			if (wd == NULL){
				ShowFatalError("inter: WisRequest: excesso de memoria !\n");
				return 0;
			}
...
int mapif_parse_AccReg(int fd) {
	...
	if (reg == NULL) {
		if ((reg = (struct accreg*)aCalloc(sizeof(struct accreg), 1)) == NULL) {
			ShowFatalError("inter: accreg: Excesso de memoria !\n");
			exit(0);
		}
