...
// nova versao do login-server por [Yor] 488 linhas traduzidas
...
int gm_account_filename_check_timer = 15; // Timer para checar se o arquivo GM_account foi modificado e reabrir o GM account automaticamente (em segundos; padrão: 15)

int log_login = 1;

int display_parse_login = 0; // 0: nao, 1: sim
int display_parse_admin = 0; // 0: nao, 1: sim
int display_parse_fromchar = 0; // 0: nao, 1: sim (sem o pacote 0x2714), 2: todos os pacotes
...
static int online_check=1; //quando 1, o login-server rejeita todos as tentativas de log dos players que estão registrados como online. [Skotlex]
//proteção de account flood [Kevin]
unsigned int new_reg_tick=0;
int allowed_regs=1;
int num_regs=0;
int time_allowed=10; //Inicie essa variavel com 10. [Skotlex]
...
int min_level_to_connect = 0; // minimo level do jogador/GM (0: jogador, 1-99: GM) para se conectar no server
int add_to_unlimited_account = 0; // dá ou naum a possibilidade  de ajustar (comando ladmin: timeadd) o tempo de contas ilimitadas.
int start_limited_time = -1; // Tempo de inicio adicial (em segundos) de agora até o tempo limite de criação de contas (-1: ilimitado, 0 ou mais: tempo adicional em segundos de agora em diante)
int check_ip_flag = 1; // Checakr o IP do jogador entre o login e o char-server(sistema anti-hack)

int check_client_version = 0; //Checkar versao do client (1-sim 2-nao) .. (sirius)
int client_version_to_connect = 20; //Versao do cliente ..(sirius)
...
struct auth_dat {
	int account_id, sex;
	char userid[24], pass[33], lastlogin[24]; // 33 para 32 + terminação nula (\0)
	int logincount;
	int state; // o pacote 0x006a valor + 1 (0: compte OK)
	char email[40]; // e-mail (padrão: a@a.com)
	char error_message[20]; // mensagem de erro codigo #6 = Você está proibido de logar até %s (pacote 0x006a)
	time_t ban_until_time; // # de segundos 1/1/1970 (timestamp): tempo limite de ban para as contas (0 =sem ban)
	time_t connect_until_time; // # de segundos 1/1/1970 (timestamp): tempo limite de validade das contas(0 = ilimitado)
	char last_ip[16]; // salvear o ultimo IP conectado
	char memo[255]; // um campo de lembrete
...
// defina o numero de vezes que os jogadores devem autenticarantes de salvar o arquivo de contas.
...
// coloque a autenticação minima antes de salvar:
#define AUTH_BEFORE_SAVE_FILE 10
// coloque o divisor do auth_num para um numero encontrado fora do tempo minimo de autenticação antes de salvar.
#define AUTH_SAVE_FILE_DIVIDER 50
int auth_before_save_file = 0; // Contador. Primoro a savar quando o primeiro conectar no char-server.
...
//------------------------------
// Escrevendo função para criar arquivos de log
//------------------------------
int login_log(char *fmt, ...) {
	if (log_login) {
		...
		if (log_fp) {
			if (fmt[0] == '\0') // pula se a linha estiver vazia
				fprintf(log_fp, RETCODE);
			else {
				va_start(ap, fmt);
				// Plataforma/compilador dependo do clock() para chegar se o tempo foi removido[Lance]
				// clock() é originalmente usado para processar as trilhas na execução do programa.(texto obscuro)
				time(&raw_time);
			...
			fflush(log_fp); // sobre cygwin ou windows, se o software estiver parado, datas não são escritas no arquivo -> fflush em cada linha
		}
	}
...
//-----------------------------------------------------
// Banco de DAdos de Usuarios online[Wizputer]
//-----------------------------------------------------
...
void remove_online_user (int account_id) {
	...
	if(!online_check)
		return;
	if (account_id == 99) {	// reset todos para offline
		numdb_final(online_db, online_db_final);	// remoção do bd
		online_db = numdb_init();	// reinicialização
		return;
	}
...
//----------------------------------------------------------------------
// Determina se uma conta é uma conta de GM
// e retprma p lvl do GM(ou 0 senão for GM ou não for encontrado)
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
// Adiciona um novo GM usando o acc id e o nivel
//----------------------------------------------------------------------
void addGM(int account_id, int level) {
	...
	for(i = 0; i < GM_num; i++)
		if (gm_account_db[i].account_id == account_id) {
			if (gm_account_db[i].level == level)
				ShowWarning("adicionando GM: a conta de GM %d foi definida duas vezes (mesmo level: %d).\n", account_id, level);
			else {
				ShowWarning("adicionando GM: a conta de GM %d foi definida duas vezes(levels: %d e %d).\n", account_id, gm_account_db[i].level, level);
	...
	if (i == GM_num && do_add) {
		...
		GM_num++;
		if (GM_num >= 4000) {
			ShowWarning("4000 contas de GM. as proximas contas de GM não serão lidas.\n");
			login_log("***AVISO: 4000 contas de GM. as proximas contas de GM não serão lidas." RETCODE);
		}
	}
}

//-------------------------------------------------------------------------
// Lendo informações do arquivo de contas de GM(e seus respectivos levels)
//-------------------------------------------------------------------------
int read_gm_account() {
	char line[512];
	FILE *fp;
	...
	if ((fp = fopen(GM_account_filename, "r")) == NULL) {
		ShowError("lendo_contas_GM: o arquivo de contas dos GM [%s] nao foi encontrado.\n", GM_account_filename);
		ShowError("                 Realmente, não há contas de GM no servidor.\n");
		login_log("lendo_contas_GM: o arquivo de contas dos GM [%s] nao foi encontrado." RETCODE, GM_account_filename);
		login_log("                 Realmente, não há contas de GM no servidor.\n" RETCODE);
		return 1;
	}

	line_counter = 0;
	// limitado a 4000, porque se mandarmos mais informação (mais de 4000 contad de GM???)
	// int (id) + int (level) = 8 bytes * 4000 = 32k (limite dos pacotes no windows)
	while(fgets(line, sizeof(line)-1, fp) && GM_num < 4000) {
		line_counter++;
		if ((line[0] == '/' && line[1] == '/') || line[0] == '\0' || line[0] == '\n' || line[0] == '\r')
			continue;
		is_range = (sscanf(line, "%d%*[-~]%d %d",&start_range,&end_range,&level)==3 ); // escala de ID [MC Cameri]
		if (!is_range && sscanf(line, "%d %d", &account_id, &level) != 2 && sscanf(line, "%d: %d", &account_id, &level) != 2)
			ShowError("lendo_contas_GM: arquivo [%s], invalido 'id da conta|escala de level' formato (linha #%d).\n", GM_account_filename, line_counter);
		else if (level <= 0)
			ShowError("read_gm_account: arquivo [%s] %d conta (linha #%d) (level invalido [0 ou negativo]: %d).\n", GM_account_filename, GM_num+1, line_counter, level);
		else {
			if (level > 99) {
				ShowNotice("lendo_contas_GM:: arquivo [%s] %d conta (level invalido, mas corrigido: %d->99).\n", GM_account_filename, GM_num+1, level);
				level = 99;
			}
			if (is_range) {
				if (start_range==end_range)
					ShowError("lendo_contas_GM:: arquivo [%s] escala invalida, o comeco da escala e igual ao final da escala (linha #%d).\n", GM_account_filename, line_counter);
				else if (start_range>end_range)
					ShowError("lendo_contas_GM:: arquivo [%s] escala invalida, o comeco da escala deve ser menor que o fim da escala (linha #%d).\n", GM_account_filename, line_counter);
				else
	...
	ShowStatus("lendo_contas_GM:: arquivo '%s' lido (%d contas de GM encontrados).\n", GM_account_filename, GM_num);
	login_log("lendo_contas_GM:: arquivo '%s' lido (%d contas de GM encontrados)." RETCODE, GM_account_filename, GM_num);

	return 0;
}

//--------------------------------------------------------------
// teste de mascara de IP
// (ip: IP para ser testado, str: mask x.x.x.x/# ou x.x.x.x/y.y.y.y)
//--------------------------------------------------------------
int check_ipmask(unsigned int ip, const unsigned char *str) {
	...
	if (sscanf((const char*)str+i, "%d.%d.%d.%d", &a0, &a1, &a2, &a3) == 4) {
		...
		ShowError("checando: mascara invalida [%s].\n", str);
		return 0;
	}

//	printf("IP testado: %08x, rede: %08x, mascara de rede: %08x\n",
...
int check_ip(unsigned int ip) {
	...
	if (access_allownum == 0 && access_denynum == 0)
		return 1; // Quando não há restrições, todos os IP são permitidos

//	+   012.345.: formulario frontal encontrado or
...
//	    Então, a notação de DNS não é autorizada para checagem de IP.
	...
	for(i = 0; i < access_allownum; i++) {
		...
		if (memcmp(access_ip, buf, strlen(access_ip)) == 0 || check_ipmask(ip, (unsigned char*)access_ip)) {
			if(access_order == ACO_ALLOW_DENY)
				return 1; // com 'permissao, não-permissao' (não-permissao não é permitido), permissão tem prioridade
			flag = ACF_ALLOW;
			break;
		}
	}
...
int mmo_auth_init(void) {
	...
	if (auth_num == 0) {
		ShowNotice("mmo_auth_init: Nenhuma conta encontrada em %s.\n", account_filename);
		sprintf(line, "Nenhuma conta encontrada em %s.", account_filename);
	} else {
		if (auth_num == 1) {
			ShowStatus("mmo_auth_init: 1 conta encontrada e lida em %s,\n", account_filename);
			sprintf(line, "1 conta encontrada e lida em %s,", account_filename);
		} else {
			ShowStatus("mmo_auth_init: %d accounts read in %s,\n", auth_num, account_filename);
			sprintf(line, "%d contas encontradas e lidas em %s,", auth_num, account_filename);
		}
		if (GM_count == 0) {
			ShowStatus("               na qual nao ha contas de GM, e ");
			sprintf(str, "%s na qual nao ha contas de GM, e", line);
		} else if (GM_count == 1) {
			ShowStatus("               na qual ha 1 conta de GM, e ");
			sprintf(str, "%s na qual ha 1 conta de GM, e", line);
		} else {
			ShowStatus("               %s na qual tem %d contas de GM, e", GM_count);
			sprintf(str, "%s na qual tem %d contas de GM, e", line, GM_count);
		}
		if (server_count == 0) {
			printf("sem contas no servidor ('S').\n");
			sprintf(line, "%s sem contas no servidor  ('S').", str);
		} else if (server_count == 1) {
			printf("1 conta no  servidor ('S').\n");
			sprintf(line, "%s 1 conta no  servidor  ('S').", str);
		} else {
			printf("%d contas no servidor ('S').\n", server_count);
			sprintf(line, "%s %d contas no servidor('S').", str, server_count);
		}
	}
...
void mmo_auth_sync(void) {
	...
	fprintf(fp, "// Arquivos de Contas: aqui sao salvos todas as informacoes sobre as contas.\n");
	fprintf(fp, "// Estrutura: ID, nome da conta, senha, horario do ultimo login, sexo, # de logins, estado, email, mensagem de erro do 7 pelo estado 7, tempo de validacao, ultimo IP de login (aceito), campo de notas, tempo de ban, repetido(texto de registro, valor de registro)\n");
	fprintf(fp, "// Algumas Explicacoes:\n");
	fprintf(fp, "//   nome da conta       : entre 4 - 23 letras para uma conta normal (cliente padrao nao pode enviar menos de 4 caracteres).\n");
	fprintf(fp, "//   senha               : entre 4 - 23 letras\n");
	fprintf(fp, "//   sex                 : M ou F para contas normais, S para contas de servidor\n");
	fprintf(fp, "//   estado              : 0: conta OK, 1 to 256: erro codigo do pacote 0x006a + 1\n");
	fprintf(fp, "//   email               : entre 3 - 29 letras (a@a.com e como se nao tivesse email)\n");
	fprintf(fp, "//   mensagem de erro    : text for the state 7: 'Your are Prohibited to login until <text>'. Max 19 char\n");
	fprintf(fp, "//   tempo de validacao  : 0: unlimited account, <other value>: date calculated by addition of 1/1/1970 + value (number of seconds since the 1/1/1970)\n");
	fprintf(fp, "//   campo de notas      : maximo de  254 letras\n");
	fprintf(fp, "//   tempo de ban        : 0: sem ban, <outro valor>: banido ate a data: data calculada pela adicao de 1/1/1970 + valor (numero de segundos desde 1/1/1970)\n");
	for(i = 0; i < auth_num; i++) {
...
//			printf("client [%s] accountpass [%s]\n", account->passwd, auth_dat[i].pass);
		...
		if ((strcmp(account->passwd, auth_dat[i].pass) && !encpasswdok)) {
			if (account->passwdenc == 0)
				...
				unsigned int j;
				p += sprintf(p, "Senha invalida (conta: %s, recebida md5[", account->userid);
				for(j = 0; j < 16; j++)
					p += sprintf(p, "%02x", ((unsigned char *)account->passwd)[j]);
				p += sprintf(p,"] calculado md5[");
				for(j = 0; j < 16; j++)
					...
					if (isGM(acc) == 0) {
						...
						if (level_new_gm > 0) {
							...
							if ((fp = fopen(GM_account_filename, "a")) != NULL) {
								...
								fprintf(fp, RETCODE "// %s: @GM comando na conta %d" RETCODE "%d %d" RETCODE, tmpstr, acc, acc, level_new_gm);
								fclose(fp);
		...
		default:
			{
				...
				if (logfp) {
					time(&raw_time);
					strftime(tmpstr, 23, date_format, localtime(&raw_time));
					fprintf(logfp, "%s: recebendo um pacote desconhecido -> desconexao" RETCODE, tmpstr);
					fprintf(logfp, "parse_fromchar: conexao #%d (ip: %s), pacote: 0x%x (with being read: %d)." RETCODE, fd, ip, RFIFOW(fd,0), RFIFOREST(fd));
					fprintf(logfp, "Detalhe (em hex):" RETCODE);
									...
									while(fgets(line, sizeof(line)-1, fp)) {
										...
										if ((line[0] == '/' && line[1] == '/') || line[0] == '\0')
											...
											if (sscanf(line, "%d %d", &GM_account, &GM_level) != 2 && sscanf(line, "%d: %d", &GM_account, &GM_level) != 2)
												...
												fprintf(fp2, "// %s: 'ladmin' Nivel de GM removido na conta %d '%s' (nivel anterior: %d)" RETCODE "//%d %d" RETCODE, tmpstr, acc, auth_dat[i].userid, GM_level, acc, new_gm_level);
												modify_flag = 1;
											} else {
												fprintf(fp2, "// %s: 'ladmin' Nivel de GM na conta %d '%s' (nivel anterior: %d)" RETCODE "%d %d" RETCODE, tmpstr, acc, auth_dat[i].userid, GM_level, acc, new_gm_level);
												modify_flag = 1;
											}
										}
									}
									if (modify_flag == 0)
										fprintf(fp2, "// %s: 'ladmin' Nivel de GM na conta %d '%s' (nivel anterior: 0)" RETCODE "%d %d" RETCODE, tmpstr, acc, auth_dat[i].userid, acc, new_gm_level);
									fclose(fp);
								} else {
		...
		default:
			{
				...
				if (logfp) {
					...
					strftime(tmpstr, 23, date_format, localtime(&raw_time));
					fprintf(logfp, "%s: recebendo um pacote desconhecido -> desconexao" RETCODE, tmpstr);
					fprintf(logfp, "parse_admin: conexao #%d (ip: %s), pacote: 0x%x (comecou lendo: %d)." RETCODE, fd, ip, RFIFOW(fd,0), RFIFOREST(fd));
					fprintf(logfp, "Detalhe (em hex):" RETCODE);
					fprintf(logfp, "---- 00-01-02-03-04-05-06-07  08-09-0A-0B-0C-0D-0E-0F" RETCODE);
					memset(tmpstr, '\0', sizeof(tmpstr));
					for(i = 0; i < RFIFOREST(fd); i++) {
		...
		default:
			if (save_unknown_packets) {
				...
				if (logfp) {
					time(&raw_time);
					strftime(tmpstr, 23, date_format, localtime(&raw_time));
					fprintf(logfp, "%s: recebendo pacote desconhecido -> desconexao" RETCODE, tmpstr);
					fprintf(logfp, "parse_login: conexao #%d (ip: %s), pacote: 0x%x (with being read: %d)." RETCODE, fd, ip, RFIFOW(fd,0), RFIFOREST(fd));
					fprintf(logfp, "Detalhe (em hex):" RETCODE);
					fprintf(logfp, "---- 00-01-02-03-04-05-06-07  08-09-0A-0B-0C-0D-0E-0F" RETCODE);
					memset(tmpstr, '\0', sizeof(tmpstr));
					for(i = 0; i < RFIFOREST(fd); i++) {
...
int parse_console(char *buf) {
	...
	if(strcmpi("shutdown", command) == 0 ||
		...
		printf("\033[32mAjuda Comandos:\033[0m\n");
		printf("  Para desligar o servidor:\n");
		printf("  'desliga|sai|qui|fim'\n");
		printf("  Para saber se o server esta vivo:\n");
		printf("  'vivo|status'\n");
	}
