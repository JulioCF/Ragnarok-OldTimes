...
static int itemdb_read_itemnametable(void)
{
	...
	aFree(buf);
	ShowStatus("Leitura feita de '"CL_WHITE"%s"CL_RESET"'.\n","data\\idnum2itemdisplaynametable.txt");
...
static int itemdb_read_itemslotcounttable(void)
{
	...
	for (p = buf; p - buf < s;){
		...
		if (slot > MAX_SLOTS)
		{
			ShowWarning("itemdb_read_itemslotcounttable: O Item %d foi especificado com %d slots, mas o servidor apenas suporta %d\n", nameid, slot, MAX_SLOTS);
			slot = MAX_SLOTS;
		}
...
static int itemdb_read_itemtrade(void)
{
	...
	fclose(fp);
	ShowStatus("Leitura feita de '"CL_WHITE"%d"CL_RESET"' entries in '"CL_WHITE"%s"CL_RESET"'.\n", ln, "item_trade.txt");
		...
		if (mysql_query(&mmysql_handle, tmp_sql) == 0) {
			...
			if (sql_res) {
				...
				while ((sql_row = mysql_fetch_row(sql_res)))
				{
					...
					{
						ShowWarning("itemdb_read_sqldb: O Item %d (%s) foi especificado com %d slots, mas o servidor apenas suporta %d\n", nameid, id->jname, id->slot, MAX_SLOTS);
						id->slot = MAX_SLOTS;
					}
	...
	for(i=0;i<2;i++){
		...
		while(fgets(line,1020,fp)){
			...
			{	//Crash-fix on broken item lines. [Skotlex]
				ShowWarning("Lendo %s: Campos insuficientes para o item com id %d, skipping.\n", filename[i], nameid);
				continue;
			}
			ln++;
			...
			if (id->slot > MAX_SLOTS)
			{
				ShowWarning("itemdb_readdb: O Item %d (%s) foi especificado com %d slots, mas o servidor apenas suporta %d\n", nameid, id->jname, id->slot, MAX_SLOTS);
				id->slot = MAX_SLOTS;
			}
