...
*/
	...
	for(i = 0; i < char_num; i++) {
		if (char_dat[i].status.char_id == p->char_id) {
			ShowError(CL_RED"mmmo_auth_init: um personagem tem a mesma id que o outro.\n");
			ShowError("               character id #%d -> novo personagem nao preparado.\n", p->char_id);
			ShowError("               Personagem salvo em arquivo."CL_RESET"\n");
			return -1;
		} else if (strcmp(char_dat[i].status.name, p->name) == 0) {
			ShowError(CL_RED"mmmo_auth_init: Ja existe personagem com este nome.\n");
	...
	if (strcmpi(wisp_server_name, p->name) == 0) {
		...
		char_log("mmo_auth_init: ******AVISO: O personagem tem o mesmo nome do servidor: Personagem '%s' = nome do servidor '%s'." RETCODE,
		          p->name, wisp_server_name);
	}
#endif //TXT_SQL_CONVERT
	if (str[next] == '\n' || str[next] == '\r')
...
int mmo_char_init(void) {
	...
	if (fp == NULL) {
		ShowError("Arquivo do personagem nao encontrado: %s.\n", char_txt);
	...
	while(fgets(line, sizeof(line)-1, fp)) {
		...
		if (ret > 0) { // negative value or zero for errors
			...
			ShowError("               -> Personagem salvo em arquivo.\n");
			switch (ret) {
			case -1:
...
int make_new_char(int fd, unsigned char *dat) {
	...
	if (char_num >= char_max) {
		...
		char_dat = (struct character_data*)aRealloc(char_dat, sizeof(struct character_data) * char_max);
		if (!char_dat) {
			ShowFatalError("Excesso de Memoria: make_new_char (realloc of char_dat).\n");
			char_log("Excesso de Memoria: make_new_char (realloc of char_dat)." RETCODE);
			exit(1);
		}
	}
		...
		case 0x2b10:
				...
				return 0;
			{
				...
				if(j < char_num)
					...
					strncpy(list[i].name, "Desconhecido", NAME_LENGTH);
	...
	while(fgets(line, sizeof(line)-1, fp)) {
		...
		if(strcmpi(w1,"timestamp_format") == 0) {
			...
			if (login_ip) {
				...
				ShowStatus("Login server Endere輟 de IP : %s -> %s\n", w2, ip_str);
			}
			...
			if (char_ip){
				...
				ShowStatus("Character server Endere輟 de IP : %s -> %s\n", w2, ip_str);
			}
			...
			if (bind_ip) {
				...
				ShowStatus("Character server ligando Endere輟 de IP : %s -> %s\n", w2, ip_str);
			}
