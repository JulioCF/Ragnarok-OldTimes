		...
		if ((pl_sd = pl_allsd[i])) {
			...
			if (!((battle_config.hide_GM_session || (pl_sd->sc.option & OPTION_INVISIBLE)) && (pl_GM_level > GM_level))) { // you can look only lower or same level
				if (pl_sd->bl.m == map_id) {
					if (pl_GM_level > 0)
						sprintf(atcmd_output, "Nome: %s (GM:%d) | Local: %s %d %d", pl_sd->status.name, pl_GM_level, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y);
					else
						sprintf(atcmd_output, "Nome: %s | Local: %s %d %d", pl_sd->status.name, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y);
					clif_displaymessage(fd, atcmd_output);
					count++;
				}
			}
	...
	for (i = 0; i < users; i++) {
		if ((pl_sd = pl_allsd[i])) {
			...
			if (!((battle_config.hide_GM_session || (pl_sd->sc.option & OPTION_INVISIBLE)) && (pl_GM_level > GM_level))) { // you can look only lower or same level
				if (pl_sd->bl.m == map_id) {
					if (pl_GM_level > 0)
						sprintf(atcmd_output, "Nome: %s (GM:%d) | Nv.Base: %d | Classe: %s (Lvl: %d)", pl_sd->status.name, pl_GM_level, pl_sd->status.base_level, job_name(pl_sd->status.class_), pl_sd->status.job_level);
					else
						sprintf(atcmd_output, "Nome: %s | Nv.Base: %d | Classe: %s (Nv: %d)", pl_sd->status.name, pl_sd->status.base_level, job_name(pl_sd->status.class_), pl_sd->status.job_level);
					clif_displaymessage(fd, atcmd_output);
					count++;
				}
			}
		...
		for (i = 0; i < users; i++) {
			...
			if ((pl_sd = pl_allsd[i]) && counted[i]==0) {
				if(pl_sd->status.zeny==zeny[c]) {
					sprintf(atcmd_output, "Nome: %s | Zeny: %d", pl_sd->status.name, pl_sd->status.zeny);
					clif_displaymessage(fd, atcmd_output);
					zeny[c]=0;
					counted[i]=1;
				}
...
int atcommand_speed(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		sprintf(atcmd_output, "Por favor, entre com um valor de velocidade válido (usando: @speed <%d-%d>).", MIN_WALK_SPEED, MAX_WALK_SPEED);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_jobchange(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (job == 13 || job == 21 || job == 24 || job == 25 || job == 26 || job == 37 ||job == 45){
...
int atcommand_go(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%15s", map_name) < 1 || town < -3 || town >= (int)(sizeof(data) / sizeof(data[0]))) {
		...
		clif_displaymessage(fd, " 15=Treinamento     16=Prisão      17=Jawaii");
		...
		if (strncmp(map_name, "prontera.gat", 3) == 0) { // 3 first characters
			...
			town = 10;
		           ...
		           strncmp(map_name, "kunlun.gat", 3) == 0) {
			town = 11;
...
int atcommand_monstersmall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (mobdb_checkid(mob_id) == 0) {
		clif_displaymessage(fd, "ID de monstro inválida"); // Invalid Monster ID.
		return -1;
	}
...
int atcommand_packet(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (strstr(command, "packetmode")) {
		...
		clif_displaymessage(fd, "Modo Packet modificado.");
		return 0;
	}
...
int atcommand_param(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &value) < 1 || value == 0) {
		sprintf(atcmd_output, "Por favor, entre com um valor válido (usando: @for,@agi,@vit,@int,@des,@sor <+/-ajustes>).");
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_guildlevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &level) < 1 || level == 0) {
		clif_displaymessage(fd, "Por favor, entre com um nível válido (usando: @guildlvup/@guildlvlup <# de níveis>).");
		return -1;
	}
...
int atcommand_petfriendly(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (friendly = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um valor válido (usando: @petfriendly <0-1000>).");
		return -1;
	}
...
int atcommand_pethungry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (hungry = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um valor válido (usando: @pethungry <0-100>).");
		return -1;
	}
	...
	if ((pl_sd = map_nick2sd(atcmd_player_name)) != NULL) {
		if (pc_isGM(sd) >= pc_isGM(pl_sd)) { // you can recall only lower or same level
			if (sd->bl.m >= 0 && map[sd->bl.m].flag.nowarpto && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
				clif_displaymessage(fd, "Você não é autorizado a teleportar alguém para seu mapa atual.");
				return -1;
			}
			if (pl_sd->bl.m >= 0 && map[pl_sd->bl.m].flag.nowarp && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
				clif_displaymessage(fd, "Você não é autorizado a teleportar este jogador deste mapa atual.");
				return -1;
			}
...
int atcommand_questskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (skill_id = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um número de quest de habilidade (usando: @questskill <#:0+>).");
		return -1;
	}
...
int atcommand_lostskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (skill_id = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um número de quest de habilidade (usando: @lostskill <#:0+>).");
		return -1;
	}
...
{
	nullpo_retr(-1, sd);
	//atcommand_broadcast( fd, sd, "@broadcast", "Servidor Cronus está recarregando..." );
	//atcommand_broadcast( fd, sd, "@broadcast", "Você sentirá um pouco de lag neste momento!" );
	//atcommand_broadcast( fd, sd, "@broadcast", "Recarregando os NPCs..." );
...
int atcommand_mapinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (list < 0 || list > 3) {
		clif_displaymessage(fd, "Por favor, entre com pelo menos um número válido da lista (usando: @mapinfo <0-3> [map]).");
		return -1;
	}
	...
	sprintf(atcmd_output, "Nome do Mapa: %s | Jogadores no Mapa: %d | NPCs no Mapa: %d | Chats no Mapa: %d", atcmd_player_name, map[m_id].users, map[m_id].npc_num, chat_num);
	clif_displaymessage(fd, atcmd_output);
	...
	case 2:
		...
		for (i = 0; i < map[m_id].npc_num;) {
			...
			sprintf(atcmd_output, "NPC %d: %s | Direção: %s | Sprite: %d | Local: %d %d",
	...
	case 3:
		...
		for (i = 0; i < users; i++) {
			if ((pl_sd = pl_allsd[i]) && (cd = (struct chat_data*)map_id2bl(pl_sd->chatID)) &&
			    ...
			    cd->usersd[0] == pl_sd) {
				sprintf(atcmd_output, "Chat %d: %s | Jogador: %s | Local: %d %d",
...
int atcommand_exp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(output, "Nv. de Base: %d (%.3f%%) | Nv. de Classe: %d (%.3f%%)", sd->status.base_level, nextb, sd->status.job_level, nextj);
...
int atcommand_character_cart_list(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if ((pl_sd = map_nick2sd(atcmd_player_name)) != NULL) {
		if (pc_isGM(sd) >= pc_isGM(pl_sd)) { // you can look items only lower or same level
			...
			if (count == 0)
				clif_displaymessage(fd, "O carrinho deste jogador não possui itens.");
			else {
				sprintf(atcmd_output, "%d item(s) encontrado(s) em %d tipo(s) de itens.", counter, count);
				clif_displaymessage(fd, atcmd_output);
			}
		} else {
			clif_displaymessage(fd, msg_txt(81)); // Your GM level don't authorise you to do this action on this player.
...
int atcommand_addwarp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(atcmd_output, "Novo Portal => %s",w3);
...
int atcommand_follow(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		...
		clif_displaymessage(fd, "Modo de Seguir Desligado.");
		return 0;
	}
	if ((pl_sd = map_nick2sd((char *) message)) != NULL) {
		if (sd->followtarget == pl_sd->bl.id) {
			...
			clif_displaymessage(fd, "Modo de Seguir Ligado.");
		}
		return 0;
	} else {
		clif_displaymessage(fd, "Personagem não encontrado.");
		return -1;
	}
...
int atcommand_chardropall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	clif_displaymessage(fd, "Concluído");
...
int atcommand_storeall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->state.storage_flag != 1)
  	{	//Open storage.
		...
		case 2: //Try again
			clif_displaymessage(fd, "executando este comando novamente..");
			return 0;
		case 1: //Failure
...
int atcommand_useskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(sscanf(message, "%d %d %99[^\n]", &skillnum, &skilllv, target) != 3) {
		clif_displaymessage(fd, "Por favor, entre com o id da habilidade, seu nível e o alvo (usando: @useskill <skillnum> <skillv> <target>)");
		return -1;
	}
...
int atcommand_skilltree(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(atcmd_output, "Jogador está usando %s skill tree (%d pontos básicos)",
	...
	if (skillidx == -1) {
		sprintf(atcmd_output, "Eu não acredito que o jogador possa usar essa habilidade");
		clif_displaymessage(fd, atcmd_output);
		return 0;
	}
	...
	for(j=0;j<5;j++)
			...
			pc_checkskill(sd,ent->need[j].id) < ent->need[j].lv)
		{
			...
			sprintf(atcmd_output, "jogador precisa de nível %d para a habilidade %s",
				ent->need[j].lv, desc);
			clif_displaymessage(fd, atcmd_output);
			meets = 0;
		}
...
int atcommand_marry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
  ...
  if (pc_marriage(pl_sd1, pl_sd2) == 0) {
	clif_displaymessage(fd, "Eles estão casados.. deseja-os o bem");
