...
static int auction_end_timer(int tid, unsigned int tick, int id, int data)
{
	...
	if( (auction = (struct auction_data *)idb_get(auction_db_, id)) != NULL )
	{
		if( auction->buyer_id )
		{
			mail_sendmail(0, "Gerenciador de Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Obrigado, você venceu o leilão!.", 0, &auction->item);
			mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
			mail_sendmail(0, "Gerenciador de Leilão", auction->seller_id, auction->seller_name, "Leilão", "Pagamento de seu leilão!.", auction->price, NULL);
		}
		else
			mail_sendmail(0, "Gerenciador de Leilão", auction->seller_id, auction->seller_name, "Leilão", "Nenhum comprador para seu leilão.", 0, &auction->item);
		
		ShowInfo("Leilão encerrado: id %u.\n", auction->auction_id);
...
static void mapif_parse_Auction_cancel(int fd)
{
	...
	mail_sendmail(0, "Gerenciador de Leilão", auction->seller_id, auction->seller_name, "Leilão", "Leilão cancelado.", 0, &auction->item);
	auction_delete(auction);
...
static void mapif_parse_Auction_close(int fd)
{
	...
	mail_sendmail(0, "Gerenciador de Leilão", auction->seller_id, auction->seller_name, "Leilão", "Negócio fechado.", auction->price, NULL);
	// Send Item to Buyer
	mail_sendmail(0, "Gerenciador de Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Vencedor do Leilão.", 0, &auction->item);
	mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
	auction_delete(auction);
...
static void mapif_parse_Auction_bid(int fd)
{
	...
	{ // Send Money back to the previous Buyer
		...
		{
			mail_sendmail(0, "Gerenciador de Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Alguém deu um lance maior.", auction->price, NULL);
			mapif_Auction_message(auction->buyer_id, 7); // You have failed to win the auction
		}
		else
			mail_sendmail(0, "Gerenciador de Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Você deu o maior lance.", auction->price, NULL);
	}
	...
	{ // Automatic won the auction
		...
		mail_sendmail(0, "Gerenciador de Leilão", auction->buyer_id, auction->buyer_name, "Leilão", "Você ganhou o leilão.", 0, &auction->item);
		mapif_Auction_message(char_id, 6); // You have won the auction
		mail_sendmail(0, "Gerenciador de Leilão", auction->seller_id, auction->seller_name, "Auction", "Pagamento de seu leilão!.", auction->buynow, NULL);
