...
void pc_paycash(struct map_session_data *sd, int price, int points)
{
	...
	if( cash < 0 ){
		if( sd->cashPoints > 0 ) {
			...
			sprintf(output, "Removed %d cash points. %d points remaining.", (cash)*(-1), sd->cashPoints);
			clif_disp_onlyself(sd, output, strlen(output));
		} else {
			sprintf(output, "No have Cash points");
			clif_disp_onlyself(sd, output, strlen(output));
		}
	}
	if( points < 0 ){
		if( sd->kafraPoints > 0 ) {
			...
			sprintf(output, "Removed %d kafra points. %d points remaining.", (points)*(-1), sd->kafraPoints);
			clif_disp_onlyself(sd, output, strlen(output));
		} else {
			sprintf(output, "No have Kafra points");
			clif_disp_onlyself(sd, output, strlen(output));
		}
	}
}
