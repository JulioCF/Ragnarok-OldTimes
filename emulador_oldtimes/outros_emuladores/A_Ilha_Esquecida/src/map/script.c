...
/// Pucha o argumento escrito entre "" mes_name(set,"<argumento>") retornando a fala do npc by [SoulBlaker]
#define mes_name(st,val) push_str((st)->stack, C_CONSTSTR, (val))
...
struct Script_Config script_config = {
	...
	"OnAuctionEvent" //auction_event_name OnAuctionEvent by [Brainstorm]
...
/*==========================================
 * str_dataÇÃíÜÇ…ñºëOÇ™Ç ÇÈÇ©åüçıÇ∑ÇÈ
 *------------------------------------------*/
...
/*==========================================
 * ÉXÉNÉäÉvÉgÉoÉbÉtÉ@ÉTÉCÉYÇÃämîFÇ∆ägí£
 *------------------------------------------*/
...
/*==========================================
 * ÉXÉNÉäÉvÉgÉoÉbÉtÉ@Ç…ÇPÉoÉCÉgèëÇ´çûÇﬁ
 *------------------------------------------*/
...
/*==========================================
 * ÉXÉNÉäÉvÉgÉoÉbÉtÉ@Ç…ÉfÅ[É^É^ÉCÉvÇèëÇ´çûÇﬁ
 *------------------------------------------*/
...
/*==========================================
 * ÉXÉNÉäÉvÉgÉoÉbÉtÉ@Ç…êÆêîÇèëÇ´çûÇﬁ
 *------------------------------------------*/
...
/*==========================================
 * ÉXÉNÉäÉvÉgÉoÉbÉtÉ@Ç…ÉâÉxÉã/ïœêî/ä÷êîÇèëÇ´çûÇﬁ
 *------------------------------------------*/
// ç≈ëÂ16MÇ‹Ç≈
static void add_scriptl(int l)
{
	...
	case C_INT:
		...
		break;
	default:
		// Ç‡Ç§ëºÇÃópìrÇ∆ämíËÇµÇƒÇÈÇÃÇ≈êîéöÇÇªÇÃÇ‹Ç‹
		add_scriptc(C_NAME);
		add_scriptb(l);
		add_scriptb(l>>8);
		add_scriptb(l>>16);
...
//		st->stack->sp=0;
	...
	if(st->state==RETFUNC){
		...
		pop_stack(st->stack,st->stack->defsp,start_sp);	// ïúãAÇ…é◊ñÇÇ»ÉXÉ^ÉbÉNçÌèú
		...
		i = conv_num(st,& (st->stack->stack_data[st->stack->defsp-5]));					// à¯êîÇÃêîèäìæ
		st->pos=conv_num(st,& (st->stack->stack_data[st->stack->defsp-1]));				// ÉXÉNÉäÉvÉgà íuÇÃïúå≥
		st->script=(struct script_code*)conv_num(st,& (st->stack->stack_data[st->stack->defsp-3]));	// ÉXÉNÉäÉvÉgÇïúå≥
		st->stack->var_function = (struct linkdb_node**)st->stack->stack_data[st->stack->defsp-2].u.num; // ä÷êîàÀë∂ïœêî

		st->stack->defsp=conv_num(st,& (st->stack->stack_data[st->stack->defsp-4]));	// äÓèÄÉXÉ^ÉbÉNÉ|ÉCÉìÉ^Çïúå≥
		pop_stack(st->stack,olddefsp-5-i,olddefsp);		// óvÇÁÇ»Ç≠Ç»Ç¡ÇΩÉXÉ^ÉbÉN(à¯êîÇ∆ïúãAópÉfÅ[É^)çÌèú
...
BUILDIN_FUNC(callfunc)
{
	...
	script_pushint(st,j);				// à¯êîÇÃêîÇÉvÉbÉVÉÖ
	script_pushint(st,st->stack->defsp);	// åªç›ÇÃäÓèÄÉXÉ^ÉbÉNÉ|ÉCÉìÉ^ÇÉvÉbÉVÉÖ
	script_pushint(st,(int)st->script);	// åªç›ÇÃÉXÉNÉäÉvÉgÇÉvÉbÉVÉÖ
	script_pushint(st,(int)st->stack->var_function);	// åªç›ÇÃä÷êîàÀë∂ïœêîÇÉvÉbÉVÉÖ
	push_val(st->stack,C_RETINFO,st->pos);		// åªç›ÇÃÉXÉNÉäÉvÉgà íuÇÉvÉbÉVÉÖ
...
/*==========================================
 * Mostra o nome da Classe defenido pelo sexo
 * 0=Feminino/1=Masculino
 * classname(Class,Sex) ou classname(1,0)
 * por [SoulBlaker]
 *------------------------------------------*/
BUILDIN_FUNC(classname)
{
	int job,type=-1;
	int class_=script_getnum(st,2);
	...
	if(type > 1)
		ShowError("nome da classe: invalido tipo, por favor, escolha entre 0 ou 1, feminino ou masculino.\n");
	else

	switch(type){
		case 0:
			switch(job){
				case 0: mes_name(st,"Aprendiz"); break;
				case 1: mes_name(st,"Espadachim"); break;
				case 2: mes_name(st,"Maga"); break;
				case 3: mes_name(st,"Arqueira"); break;
				case 4: mes_name(st,"NoviÁa"); break;
				case 5: mes_name(st,"Mercadora"); break;
				case 6: mes_name(st,"Gatuna"); break;
				case 7: case 13: mes_name(st,"Cavaleira"); break;
				case 8: mes_name(st,"Sacerdotisa"); break;
				case 9: mes_name(st,"Bruxa"); break;
				case 10: mes_name(st,"Ferreira"); break;
				case 11: mes_name(st,"CaÁadora"); break;
				case 12: mes_name(st,"Mercen·ria"); break;
				case 14: case 21: mes_name(st,"Templ·ria"); break;
				case 15: mes_name(st,"Monja"); break;
				case 16: mes_name(st,"S·bia"); break;
				case 17: mes_name(st,"Arruaceira"); break;
				case 18: mes_name(st,"Alquimista"); break;
				case 19: case 20: mes_name(st,"Odalisca"); break;
				case 23: mes_name(st,"Super Aprendiz"); break;
				case 24: mes_name(st,"Justiceira"); break;
				case 25: mes_name(st,"Ninja"); break;
				case 4001: mes_name(st,"Aprendiz T."); break;
				case 4002: mes_name(st,"Espadachim T."); break;
				case 4003: mes_name(st,"Maga T."); break;
				case 4004: mes_name(st,"Arqueira T."); break;
				case 4005: mes_name(st,"NoviÁa T."); break;
				case 4006: mes_name(st,"Mercadora T."); break;
				case 4007: mes_name(st,"Gatuna T."); break;
				case 4008: case 4014: mes_name(st,"Lady"); break;
				case 4009: mes_name(st,"Suma Sacerdotisa"); break;
				case 4010: mes_name(st,"Arquimaga"); break;
				case 4011: mes_name(st,"Mestra Ferreira"); break;
				case 4012: mes_name(st,"Atiradora de Elite"); break;
				case 4013: mes_name(st,"Algoneza"); break;
				case 4015: case 4022: mes_name(st,"Paladina"); break;
				case 4016: mes_name(st,"Mestra"); break;
				case 4017: mes_name(st,"Professora"); break;
				case 4018: mes_name(st,"Desordeira"); break;
				case 4019: mes_name(st,"Criadora"); break;
				case 4020: case 4021: mes_name(st,"Cigana"); break;
				case 4023: mes_name(st,"BebÍ Aprendiz"); break;
				case 4024: mes_name(st,"BebÍ Espadachim"); break;
				case 4025: mes_name(st,"BebÍ Maga"); break;
				case 4026: mes_name(st,"BebÍ Arqueira"); break;
				case 4027: mes_name(st,"BebÍ NoviÁa"); break;
				case 4028: mes_name(st,"BebÍ Mercadora"); break;
				case 4029: mes_name(st,"BebÍ Gatuna"); break;
				case 4030: case 4036: mes_name(st,"BebÍ Cavaleira"); break;
				case 4031: mes_name(st,"BebÍ Sacerdotisa"); break;
				case 4032: mes_name(st,"BebÍ Bruxa"); break;
				case 4033: mes_name(st,"BebÍ Ferreira"); break;
				case 4034: mes_name(st,"BebÍ CaÁadora"); break;
				case 4035: mes_name(st,"BebÍ Mercen·ria"); break;
				case 4037: mes_name(st,"BebÍ Templ·ria"); break;
				case 4038: case 4044: mes_name(st,"BebÍ Monja"); break;
				case 4039: mes_name(st,"BebÍ S·bia"); break;
				case 4040: mes_name(st,"BebÍ Arruaceira"); break;
				case 4041: mes_name(st,"BebÍ Alquimista"); break;
				case 4042: case 4043: mes_name(st,"BebÍ Odalisca"); break;
				case 4045: mes_name(st,"BebÍ S.Aprendiz"); break;
				case 4046: mes_name(st,"Taekwon"); break;
				case 4047: case 4048: mes_name(st,"Mestra Taekwon"); break;
				case 4049: mes_name(st,"Espiritualista");
				default: mes_name(st,"Desconhecida"); break;
			}
			return 0;
		case 1:
			switch(job){
				case 0: mes_name(st,"Aprendiz"); break;
				case 1: mes_name(st,"Espadachim"); break;
				case 2: mes_name(st,"Mago"); break;
				case 3: mes_name(st,"Arqueiro"); break;
				case 4: mes_name(st,"NoviÁo"); break;
				case 5: mes_name(st,"Mercador"); break;
				case 6: mes_name(st,"Gatuno"); break;
				case 7: case 13: mes_name(st,"Cavaleiro"); break;
				case 8: mes_name(st,"Sacerdote"); break;
				case 9: mes_name(st,"Bruxo"); break;
				case 10: mes_name(st,"Ferreiro"); break;
				case 11: mes_name(st,"CaÁador"); break;
				case 12: mes_name(st,"Mercen·rio"); break;
				case 14: case 21: mes_name(st,"Templ·rio"); break;
				case 15: mes_name(st,"Monge"); break;
				case 16: mes_name(st,"S·bio"); break;
				case 17: mes_name(st,"Arruaceiro"); break;
				case 18: mes_name(st,"Alquimista"); break;
				case 19: case 20: mes_name(st,"Bardo"); break;
				case 23: mes_name(st,"Super Aprendiz"); break;
				case 24: mes_name(st,"Justiceiro"); break;
				case 25: mes_name(st,"Ninja"); break;
				case 4001: mes_name(st,"Aprendiz T."); break;
				case 4002: mes_name(st,"Espadachim T."); break;
				case 4003: mes_name(st,"Mago T."); break;
				case 4004: mes_name(st,"Arqueiro T."); break;
				case 4005: mes_name(st,"NoviÁo T."); break;
				case 4006: mes_name(st,"Mercador T."); break;
				case 4007: mes_name(st,"Gatuno T."); break;
				case 4008: case 4014: mes_name(st,"Lorde"); break;
				case 4009: mes_name(st,"Sumo Sacerdote"); break;
				case 4010: mes_name(st,"Arquimago"); break;
				case 4011: mes_name(st,"Mestre Ferreiro"); break;
				case 4012: mes_name(st,"Atirador de Elite"); break;
				case 4013: mes_name(st,"Algoz"); break;
				case 4015: case 4022: mes_name(st,"Paladino"); break;
				case 4016: mes_name(st,"Mestre"); break;
				case 4017: mes_name(st,"Professor"); break;
				case 4018: mes_name(st,"Desordeiro"); break;
				case 4019: mes_name(st,"Criador"); break;
				case 4020: case 4021: mes_name(st,"Menestrel"); break;
				case 4023: mes_name(st,"BebÍ Aprendiz"); break;
				case 4024: mes_name(st,"BebÍ Espadachim"); break;
				case 4025: mes_name(st,"BebÍ Mago"); break;
				case 4026: mes_name(st,"BebÍ Arqueiro"); break;
				case 4027: mes_name(st,"BebÍ NoviÁo"); break;
				case 4028: mes_name(st,"BebÍ Mercador"); break;
				case 4029: mes_name(st,"BebÍ Gatuno"); break;
				case 4030: case 4036: mes_name(st,"BebÍ Cavaleiro"); break;
				case 4031: mes_name(st,"BebÍ Sacerdote"); break;
				case 4032: mes_name(st,"BebÍ Bruxo"); break;
				case 4033: mes_name(st,"BebÍ Ferreiro"); break;
				case 4034: mes_name(st,"BebÍ CaÁador"); break;
				case 4035: mes_name(st,"BebÍ Mercen·rio"); break;
				case 4037: mes_name(st,"BebÍ Templ·rio"); break;
				case 4038: case 4044: mes_name(st,"BebÍ Monge"); break;
				case 4039: mes_name(st,"BebÍ S·bio"); break;
				case 4040: mes_name(st,"BebÍ Arruaceiro"); break;
				case 4041: mes_name(st,"BebÍ Alquimista"); break;
				case 4042: case 4043: mes_name(st,"BebÍ Bardo"); break;
				case 4045: mes_name(st,"BebÍ S.Aprendiz"); break;
				case 4046: mes_name(st,"Taekwon"); break;
				case 4047: case 4048: mes_name(st,"Mestre Taekwon"); break;
				case 4049: mes_name(st,"Espiritualista");
				default: mes_name(st,"Desconhecido"); break;
			}
			return 0;
		default:
		ShowError("nome da classe: invalido tipo, por favor, escolha entre 0 ou 1, feminino ou masculino.\n");
...
BUILDIN_FUNC(checkjob)
{
	...
	switch(type){
		case 1: if(job >= 1 && job <= 6) type=1; else type=0; break;
		case 2: if(job >= 7 && job <= 21) type=1; else type=0; break;
		case 3: if(job >= 4002 && job <= 4007) type=1; else type=0; break;
		case 4: if(job >= 4008 && job <= 4022) type=1; else type=0; break;
		case 5: if(job >= 4024 && job <= 4029) type=1; else type=0; break;
		case 6: if(job >= 4030 && job <= 4044) type=1; else type=0; break;
		case 7: if(job >= 24 && job <= 25) type=1; else type=0; break;
		default: type=0;
	}
	if(type == 1)
		script_pushint(st, 1);
...
BUILDIN_FUNC(additem2)
{
	...
	if( script_hasdata(st,11) ) //ÉAÉCÉeÉÄÇéwíËÇµÇΩIDÇ…ìnÇ∑
		sd=map_id2sd(script_getnum(st,11));
	if(sd == NULL) //ÉAÉCÉeÉÄÇìnÇ∑ëäéËÇ™Ç¢Ç»Ç©Ç¡ÇΩÇÁÇ®ãAÇË
		return 0;
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(setjob,"i*"),
	BUILDIN_DEF(jobname,"i"),
	BUILDIN_DEF(classname,"i*"), // by SoulBlaker
	BUILDIN_DEF(checkjob,"i*"), // by SoulBlaker
	BUILDIN_DEF(digit,"v??"),
	...
	BUILDIN_DEF(additem,"vi?"),
	BUILDIN_DEF(additem2,"iiiiiiiii*"),
	...
	//Comandos Cronus
	/* InÌcio Compatibilidade com o eA [Nicko] */
	BUILDIN_DEF2(show,"mes","s"),
	BUILDIN_DEF2(setjob,"jobchange","i*"),
	BUILDIN_DEF2(digit,"input","v"),
	BUILDIN_DEF2(additem,"getitem","vi?"),
	BUILDIN_DEF2(additem2,"getitem2","iiiiiiiii*"),
	BUILDIN_DEF2(setbase,"changebase","i"),
	BUILDIN_DEF2(setsex,"changesex",""),
	/* Fim Compatibilidade com o eA [Nicko] */
#ifdef PCRE_SUPPORT
