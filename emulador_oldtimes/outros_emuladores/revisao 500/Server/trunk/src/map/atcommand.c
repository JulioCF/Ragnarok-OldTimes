	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (usando: @where <nom do personagem>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (usando: @jumpto/@warpto/@goto <nome do personagem>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message) {
		sprintf(atcmd_output, "Por favor, entre com um valor de velocidade v·lido (usando: @speed <%d-%d>).", MIN_WALK_SPEED, MAX_WALK_SPEED);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &param1, &param2, &param3) < 1 || param1 < 0 || param2 < 0 || param3 < 0) {
		clif_displaymessage(fd, "Por favor, entre com pelo menos uma opÁ„o (usando: @option <param1:0+> <param2:0+> <param3:0+>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1) {
		...
		if (!found) {
			clif_displaymessage(fd, "Por favor, entre com a ID da Classe (usando: @job/@jobchange <ID da Classe>).");
			return -1;
		}
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do personagem (usando: @kill <nome do personagem>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if(*(command + 5) != 'c' && *(command + 5) != 'C') {
		...
		if (!message || !*message) {
			clif_displaymessage(fd, "Por favor, entre com uma mensagem (usando: @kami <mensagem>).");
			return -1;
		}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s %d", item_name, &number) < 1) {
		clif_displaymessage(fd, "Por favor, entre com um nome de item ou id (usando: @item <nome do item ou ID> [quantidade]).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s %d %d %d %d %d %d %d %d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4) < 9) {
		clif_displaymessage(fd, "Por favor, entre com todas as informaÁıes (usando: @item2 <nome do item ou ID> <quantidade>");
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, "Por favor, entre com um nÌvel (usando: @lvup/@blevel/@baselvlup <n˙mero de nÌveis>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", password) < 1) {
		clif_displaymessage(fd, "Por favor, entre com uma senha (usando: @gm <senha>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &hair_style, &hair_color, &cloth_color) < 1) {
		sprintf(atcmd_output, "Por favor, entre com pelo menos um valor (usando: @model <ID do cabelo: %d-%d> <cor do cabelo: %d-%d> <cor da roupa: %d-%d>).",
...
int atcommand_dye(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &cloth_color) < 1) {
		sprintf(atcmd_output, "Por favor, entre com um cor da roupa (usando: @dye/@ccolor <cor da roupa: %d-%d>).", MIN_CLOTH_COLOR, MAX_CLOTH_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_hair_style(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &hair_style) < 1) {
		sprintf(atcmd_output, "Por favor, entre com um estilo de cabelo (usando: @hairstyle/@hstyle <ID do cabelo: %d-%d>).", MIN_HAIR_STYLE, MAX_HAIR_STYLE);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_hair_color(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &hair_color) < 1) {
		sprintf(atcmd_output, "Por favor, entre com uma cor de cabelo (usando: @haircolor/@hcolor <cor do cabelo: %d-%d>).", MIN_HAIR_COLOR, MAX_HAIR_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if(map[sd->bl.m].flag.nogo) {
		clif_displaymessage(sd->fd,"VocÍ n„o pode usar #go neste mapa.");
		return 0;
	}
	...
	if (!message || !*message || sscanf(message, "%15s", map_name) < 1 || town < -3 || town >= (int)(sizeof(data) / sizeof(data[0]))) {
		...
		clif_displaymessage(fd, " 6=Al De Baran      7=Comodo"); //7=Lutie
		clif_displaymessage(fd, " 8=Juno             9=Amatsu      10=Kunlun");
		clif_displaymessage(fd, " 11=Umbala          12=Niflheim");
		clif_displaymessage(fd, " 13=Campo de Treinamento  14=Pris„o"); //14=Lou Yang
...
int atcommand_monstersmall(
	...
	if (mobdb_checkid(mob_id) == 0) {
		clif_displaymessage(fd, "ID de monstro inv·lida"); // Invalid Monster ID.
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &position, &refine) < 2) {
		clif_displaymessage(fd, "Por favor, entre com a posiÁ„o e uma quantidade (usando: @refine <posiÁ„o do equip> <+/- quantidade>).");
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s %d %d", item_name, &attribute, &star) < 1) {
		clif_displaymessage(fd, "Por favor, entre com pelo menos um nome/id do item (usando: @produce <nome do equip ou ID do equip> <elemento> <# of very's>).");
		return -1;
	}
