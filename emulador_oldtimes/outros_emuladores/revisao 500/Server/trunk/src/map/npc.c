...
static struct tm ev_tm_b;	// éûåvÉCÉxÉìÉgóp
...
int npc_event_dequeue(struct map_session_data *sd)
{
	...
	sd->npc_id=0;
	if (sd->eventqueue[0][0]) {	// ÉLÉÖÅ[ÇÃÉCÉxÉìÉgèàóù
		size_t ev;
...
/*==========================================
 * ÉCÉxÉìÉgópÉâÉxÉãÇÃÉGÉNÉXÉ|Å[Ég
...
//				printf("npc_event: enqueue\n");
	...
	if (ev->nd->sc.option&OPTION_INVISIBLE) {	// ñ≥å¯âªÇ≥ÇÍÇƒÇ¢ÇÈ
...
int npc_touch_areanpc(struct map_session_data *sd,int m,int x,int y)
{
	...
	for(i=0;i<map[m].npc_num;i++) {
		if (map[m].npc[i]->sc.option&OPTION_INVISIBLE) {	// ñ≥å¯âªÇ≥ÇÍÇƒÇ¢ÇÈ
			f=0;
			continue;
		}
...
int npc_checknear(struct map_session_data *sd,int id)
{
	...
	if (nd->class_<0)	// ÉCÉxÉìÉgånÇÕèÌÇ…OK
		return 0;
...
int npc_buylist(struct map_session_data *sd,int n,unsigned short *item_list)
{
	...
	for(i=0;i<n;i++) {
		...
		item_tmp.identify = 1;	// npcîÃîÑÉAÉCÉeÉÄÇÕä”íËçœÇ›
...
static int npc_parse_script (char *w1,char *w2,char *w3,char *w4,char *first_line,FILE *fp,int *lines)
{
	...
	// ÉâÉxÉãÉfÅ[É^ÇÃèÄîı
	if (srcbuf){
		// scriptñ{ëÃÇ™Ç ÇÈèÍçáÇÃèàóù
		// ÉâÉxÉãÉfÅ[É^ÇÃÉRÉìÉoÅ[Ég
		label_db = script_get_label_db();
		label_db->foreach(label_db, npc_convertlabel_db, nd);

		// Ç‡Ç§égÇÌÇ»Ç¢ÇÃÇ≈ÉoÉbÉtÉ@âï˙
		aFree(srcbuf);
	} else {
		// duplicate
		nd->u.scr.label_list = label_dup;	// ÉâÉxÉãÉfÅ[É^ã§óL
...
void npc_parsesrcfile (char *name)
{
	...
	while (fgets(line, sizeof(line) - 1, fp)) {
		...
		if (line[0] == '/' && line[1] == '/')
			continue;
		// ïsóvÇ»ÉXÉyÅ[ÉXÇ‚É^ÉuÇÃòAë±ÇÕãlÇﬂÇÈ
		for (i = j = 0; line[i]; i++) {
			if (line[i]==' ') {
		...
		// ç≈èâÇÕÉ^ÉuãÊêÿÇËÇ≈É`ÉFÉbÉNÇµÇƒÇ›ÇƒÅAÉ_ÉÅÇ»ÇÁÉXÉyÅ[ÉXãÊêÿÇËÇ≈ämîF
