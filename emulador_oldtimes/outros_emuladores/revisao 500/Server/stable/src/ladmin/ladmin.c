	...
	while(RFIFOREST(fd) >= 2) {
		...
		case 0x7531:	// Displaying of the version of the login-server
			...
			if (((int)RFIFOB(login_fd,5)) == 0) {
				printf("  Cronus version stable-%d.%d", (int)RFIFOB(login_fd,2), (int)RFIFOB(login_fd,3));
			} else {
				printf("  Cronus version dev-%d.%d", (int)RFIFOB(login_fd,2), (int)RFIFOB(login_fd,3));
			}
			...
			if(strcmpi(w1,"login_ip")==0){
				...
				if (h != NULL) {
					if (defaultlanguage == 'F') {
						...
						printf("Login server Endereço de IP: %s -> %d.%d.%d.%d\n", w2, (unsigned char)h->h_addr[0], (unsigned char)h->h_addr[1], (unsigned char)h->h_addr[2], (unsigned char)h->h_addr[3]);
					}
...
int do_init(int argc, char **argv)
{
	...
	if (defaultlanguage == 'F') {
		printf("Outil d'administration à distance de Cronus.\n");
		printf("(pour Cronus version %d.%d.%d.)\n", ATHENA_MAJOR_VERSION, ATHENA_MINOR_VERSION, ATHENA_REVISION);
	} else {
		printf("Cronus login-server administration tool.\n");
		printf("(for Cronus version %d.%d.%d.)\n", ATHENA_MAJOR_VERSION, ATHENA_MINOR_VERSION, ATHENA_REVISION);
	}
