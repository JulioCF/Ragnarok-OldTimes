...
int inter_pet_init()
{
	...
	while(fgets(line,sizeof(line),fp)){
		...
		if(p==NULL){
			ShowFatalError("int_pet: Falta de memoria!\n");
			exit(0);
		}
		memset(p,0,sizeof(struct s_pet));
		if(inter_pet_fromstr(line,p)==0 && p->pet_id>0){
			...
			ShowError("int_pet: Arquivo corrompido [%s] na linha %d\n",pet_txt,c);
			aFree(p);
		}
		c++;
	}
...
int inter_pet_save()
{
	...
	if( (fp=lock_fopen(pet_txt,&lock))==NULL ){
		ShowError("int_pet: escrita impossivel [%s] !!! arquivo corrompido !!!\n",pet_txt);
		return 1;
	}
...
int inter_pet_delete(int pet_id)
{
	...
	if( p == NULL)
		...
		ShowInfo("Pet Deletado (pet_id: %d)\n",pet_id);
	}
	return 0;
}
...
int mapif_save_pet(int fd,int account_id,struct s_pet *data)
{
	...
	if(sizeof(struct s_pet)!=len-8) {
		ShowError("inter pet: erro tamanho do arquivo %d %d\n",sizeof(struct s_pet),len-8);
	}
	else{
		pet_id = data->pet_id;
		p=(struct s_pet *)numdb_search(pet_db,pet_id);
