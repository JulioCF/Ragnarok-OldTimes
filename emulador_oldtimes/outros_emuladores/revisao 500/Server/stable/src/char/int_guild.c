...
int inter_guild_readdb() {
	...
	fp = fopen(path, "r");
	if (fp == NULL) {
		ShowError("impossivel ler db/exp_guild.txt\n");
		return 1;
	}
	i = 0;
...
int inter_guild_init() {
	...
	while(fgets(line, sizeof(line)-1, fp)) {
		...
		if(g == NULL){
			ShowFatalError("int_guild: despejo de memoria!\n");
			exit(0);
		}
//		memset(g, 0, sizeof(struct guild)); not needed...
		if (inter_guild_fromstr(line, g) == 0 && g->guild_id > 0) {
			...
			ShowError("int_guild: data danificada [%s] linha %d\n", guild_txt, c);
			aFree(g);
		}
		c++;
	}
...
//		memset(gc, 0, sizeof(struct guild_castle)); No need...
	...
	if (!c) {
		ShowStatus(" %s - consruindo Default Data...\n", castle_txt);
		//デフォルトデータを作成
		for(i = 0; i < MAX_GUILDCASTLE; i++) {
			gc = (struct guild_castle *) aCalloc(sizeof(struct guild_castle), 1);
			if (gc == NULL) {
				ShowFatalError("int_guild: excesso de memoria!\n");
				exit(0);
			}
			gc->castle_id = i;
			numdb_insert(castle_db, gc->castle_id, gc);
		}
		ShowStatus(" %s - construcao completa\n",castle_txt);
		return 0;
	}
...
int inter_guild_save() {
	...
	if ((fp = lock_fopen(guild_txt, &lock)) == NULL) {
		ShowError("int_guild: impossivel ler [%s] !!! data esta perdida !!!\n", guild_txt);
		return 1;
	}
...
int guild_check_conflict_sub(void *key, void *data, va_list ap) {
	...
	for(i = 0; i < MAX_GUILD; i++) {
		if (g->member[i].account_id == account_id && g->member[i].char_id == char_id) {
			...
			ShowWarning("int_guild: conflito de guildas! %d,%d %d!=%d\n", account_id, char_id, guild_id, g->guild_id);
...
int guild_calcinfo(struct guild *g) {
	...
	{	
		ShowError("Guilda %d:%s ja possui muitos membros (%d), o maximo suportado e %d\n", g->guild_id, g->name, g->max_member, MAX_GUILD);
...
int mapif_guild_noinfo(int fd, int guild_id) {
	...
	WFIFOSET(fd,8);
	ShowNotice("int_guild: informacao nao encontrada %d\n", guild_id);
...
//	mapif_sendall(buf, 79);
	ShowInfo("Personagem saiu da guida (Guild %d, %d - %s: %s)\n", guild_id, account_id, name, mes);
...
int mapif_parse_CreateGuild(int fd, int account_id, char *name, struct guild_member *master) {
	...
	for(i = 0; i < NAME_LENGTH && name[i]; i++) {
		if (!(name[i] & 0xe0) || name[i] == 0x7f) {
			ShowInfo("Criar guildas: nome de guilda ilegal [%s]\n", name);
	...
	if ((g = search_guildname(name)) != NULL) {
		ShowInfo("Criar guildas: este nome ja existe [%s]\n", name);
		mapif_guild_created(fd, account_id, NULL);
		return 0;
	}
	g = (struct guild *) aCalloc(sizeof(struct guild), 1);
	if (g == NULL) {
		ShowFatalError("int_guild: Criarguildas: despejo de memoria !\n");
		mapif_guild_created(fd, account_id, NULL);
		exit(0);
	}
//	memset(g, 0, sizeof(struct guild)); Meh...
	...
	for(i = 1; i < MAX_GUILDPOSITION-1; i++)
		sprintf(g->position[i].name, "Posicao %d", i + 1);
	...
	if(log_inter)
		inter_log("guilda %s (id=%d) criada pelo lider %s (id=%d)" RETCODE,
...
int mapif_parse_BreakGuild(int fd, int guild_id) {
	...
	if(log_inter)
		inter_log("guilda %s (id=%d) desfeita" RETCODE, g->name, guild_id);
	aFree(g);
...
int mapif_parse_GuildMemberInfoChange(int fd, int guild_id, int account_id, int char_id, int type, const char *data, int len) {
	...
	for(i = 0; i < g->max_member; i++)
		if (g->member[i].account_id == account_id && g->member[i].char_id == char_id)
			break;
	if (i == g->max_member) {
		ShowWarning("int_guild: GuildMemberChange: Nao encontrado %d,%d in %d[%s]\n", account_id, char_id, guild_id, g->name);
		return 0;
	}
	switch(type) {
...
int mapif_parse_GuildPosition(int fd, int guild_id, int idx, struct guild_position *p) {
	...
	ShowInfo("int_guild: posicao [%d] modificada\n", idx);
...
int mapif_parse_GuildSkillUp(int fd, int guild_id, int skill_num, int account_id) {
	...
	if (g->skill_point > 0 && g->skill[idx].id > 0 && g->skill[idx].lv < 10) {
		...
		ShowInfo("int_guild: habilidade %d up\n", skill_num);
	}
...
int mapif_parse_GuildMasterChange(int fd, int guild_id, const char* name, int len)
{
	...
	ShowInfo("int_guild: Guildmaster trocado to %s (Guilda %d - %s)\n",name, guild_id, g->name);
...
int inter_guild_leave(int guild_id, int account_id, int char_id) {
	return mapif_parse_GuildLeave(-1, guild_id, account_id, char_id, 0, "** Personagem deletado **");
}
//Tradu軋o por Snails (cronus Core Dev.)
