		...
		if(c==0){ // replace
			...
			if(p->deleted) {
				...
				if(i == table->free_count || table->free_count <= 0) {
					ShowError("db_insert: impossivel encontrar no deletado da db.\n");
				} else {
					table->free_count--;
					if(table->cmp == strdb_cmp) {
						aFree(p->key);
...
#endif
	if(p==NULL){
		ShowFatalError("despejo de memoria : db_insert\n");
		return NULL;
	}
	p->parent= NULL;
	p->left  = NULL;
...
void db_foreach(struct dbt *table,int (*func)(void*,void*,va_list),...)
{
	...
	if(count) {
		ShowError(
			"db_foreach : data perdida %d item(s) foram alocados em %s linha %d\n",
