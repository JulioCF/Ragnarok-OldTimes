...
//	int ret = 0;
	...
	switch (flag) {
		...
		case MSG_WARNING: //Bright Yellow
			strcat(prefix,CL_YELLOW"[Atenção]"CL_RESET":");
			break;
		case MSG_DEBUG: //Bright Cyan, important stuff!
			strcat(prefix,CL_CYAN"[Erro em debugação]"CL_RESET":");
			break;
		...
		case MSG_FATALERROR: //Bright Red (Fatal errors, abort(); if possible)
			...
			break;
		default:
			ShowError("Em função _vShowMessage() -> Flag Invalida.\n");
			return 1;
	}
	...
	if(strlen(DEBUGLOGPATH) > 0) {
		...
		if (fp == NULL)	{
			printf(CL_RED"[ERROR]"CL_RESET": Não foi possivel abrir '"CL_WHITE"%s"CL_RESET"', Permissão negada.\n",DEBUGLOGPATH);
			fflush(stdout);
			return 0;
		}
		fprintf(fp,"%s ", prefix);
		...
		printf(CL_RED"[Erro]"CL_RESET": DEBUGLOGPATH Não Definido!\n");
	}
#endif
...
/*
	if ((core_config.debug_output_level > -1) && (flag >= core_config.debug_output_level)) {
		...
		if (fp == NULL)	{
			ShowError("Não foi possivel abrir '"CL_WHITE"%s"CL_RESET"', Arquivo não está sendo localizado.\n",OUTPUT_MESSAGES_LOG);
			fflush(stdout);
			return;
		}
		StripColor(output);
