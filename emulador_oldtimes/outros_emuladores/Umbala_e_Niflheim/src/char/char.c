...
int mmo_char_fromstr(char *str, struct mmo_charstatus *p) {
	...
	for(i = 0; i < char_num; i++) {
		if (char_dat[i].char_id == p->char_id) {
			...
			ShowError("               character id #%d -> .\n", p->char_id);
			ShowError("               Personagem salvo em arquivo.\033[0m\n");
			return -1;
	...
	if (strcmpi(wisp_server_name, p->name) == 0) {
		...
		ShowWarning("               Character readed. Suggestion: change the nome do servidor.\n");
...
int parse_friend_txt(struct mmo_charstatus *p)
{
	...
	/*
		//Personagems must not exceed the 23+\0 limit. [Skotlex]
...
int mmo_char_init(void) {
	...
	if (fp == NULL) {
		...
		char_log("ID do personagem nao encontrado: %s." RETCODE, char_txt);
