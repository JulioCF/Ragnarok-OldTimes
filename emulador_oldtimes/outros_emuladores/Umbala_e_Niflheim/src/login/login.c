...
int check_ip_flag = 1; // Checar o IP do jogador entre o login e o char-server(sistema anti-hack)

int check_client_version = 0; //Checar versao do client (1-sim 2-nao) .. (sirius)
...
int read_gm_account() {
	...
	if ((fp = fopen(GM_account_filename, "r")) == NULL) {
		...
		login_log("                 Realmente, não há contas de GM no servidor." RETCODE);
		return 1;
	}
...
//--------------------------------
// Controle de Acesso por IP for ladmin
//--------------------------------
	...
	while(RFIFOREST(fd) >= 2) {
		...
		case 0x7942:	// Request to modify memo field
			...
			if (i != -1) {
				...
				WFIFOL(fd,2) = auth_dat[i].account_id;
				login_log("'ladmin': Modification of um campo de lembrete (account: %s, new memo: %s, ip: %s)" RETCODE,
	...
	while(fgets(line, sizeof(line)-1, fp)) {
		...
		if (sscanf(line, "%[^:]: %[^\r\n]", w1, w2) == 2) {
			...
			if(strcmpi(w1,"timestamp_format") == 0) {
				...
				display_parse_login = config_switch(w2); // 0: nao, 1: sim
			} else if (strcmpi(w1, "display_parse_admin") == 0) {
				display_parse_admin = config_switch(w2); // 0: nao, 1: sim
			} else if (strcmpi(w1, "display_parse_fromchar") == 0) {
				display_parse_fromchar = config_switch(w2); // 0: nao, 1: sim (sem o pacote 0x2714), 2: todos os pacotes
...
void display_conf_warnings(void) {
	...
	if (display_parse_login != 0 && display_parse_login != 1) { // 0: nao, 1: sim
		ShowWarning("Invalid value for display_parse_login parameter -> setting to 0 (no display).\n");
		display_parse_login = 0;
	}

	if (display_parse_admin != 0 && display_parse_admin != 1) { // 0: nao, 1: sim
		ShowWarning("Invalid value for display_parse_admin parameter -> setting to 0 (no display).\n");
		display_parse_admin = 0;
	}

	if (display_parse_fromchar < 0 || display_parse_fromchar > 2) { // 0: nao, 1: sim (sem o pacote 0x2714), 2: todos os pacotes
	...
	if (add_to_unlimited_account != 0 && add_to_unlimited_account != 1) { // 0: nao, 1: sim
	...
	if (check_ip_flag != 0 && check_ip_flag != 1) { // 0: nao, 1: sim
