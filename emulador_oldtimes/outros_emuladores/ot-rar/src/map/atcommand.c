...
ACMD_FUNC(jump)
{
	...
	if( pc_isdead(sd) )
	{
		clif_displaymessage(fd, "Você não pode utilizar este comando estando morto.");
		return -1;
	}
...
ACMD_FUNC(load)
{
	...
	if (sd->status.save_point.map ==  mapindex_name2id(MAP_NOVICE)) {
...
ACMD_FUNC(storage)
{
	...
	if (agit_flag == 1) {
		clif_displaymessage(fd, "Você não pode utilizar o armazém durante o período de guerras.");
		return -1;
	}
	
	if(pc_readaccountreg(sd,"#kafra_code") && pc_readaccountreg(sd,"#kafra_code") != atoi(message)){
		clif_displaymessage(fd,"@storage falhou. Modo de Usar: @storage <senha>");
		return -1;
	}
	...
	if(!pc_readaccountreg(sd,"#kafra_code"))
		clif_displaymessage(fd, "Você ainda não possui uma senha para seu armazém. Peça a uma Funcionária Kafra para criar uma!");
...
ACMD_FUNC(die)
{
	...
	if (agit_flag == 1) {
		clif_displaymessage(fd, "Você não pode utilizar este comando durante o período de guerras.");
		return -1;
	}	
...
ACMD_FUNC(vip)
{
	if (pc_isGM(sd) < 1)
	{
		clif_displaymessage(fd, "Este comando é exclusivo dos VIPs.");
		return -1;
	}
	
	clif_displaymessage(fd, msg_txt(0));
	pc_setpos(sd, mapindex_name2id(MAP_VIP), 169, 164, CLR_TELEPORT);
	return 0;
}
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_PRONTERA,    156, 177 }, //  0=Prontera
		{ MAP_MORROC,      156,  93 }, //  1=Morroc
		{ MAP_GEFFEN,      119,  59 }, //  2=Geffen
		{ MAP_PAYON,        89, 112 }, //  3=Payon
		{ MAP_ALBERTA,     116,  69 }, //  4=Alberta
		{ MAP_IZLUDE,      128, 114 }, //  5=Izlude
		{ MAP_ALDEBARAN,   140, 131 }, //  6=Al de Baran
		{ MAP_LUTIE,       147, 134 }, //  7=Lutie
		{ MAP_COMODO,      209, 143 }, //  8=Comodo
		{ MAP_YUNO,        157, 168 }, //  9=Yuno
		{ MAP_AMATSU,      113, 149 }, // 10=Amatsu
		{ MAP_GONRYUN,     160, 120 }, // 11=Gonryun
		{ MAP_VIP,		   169, 164 }, // 12=VIP Room
	/*
	...
	*/
	...
	if( map[sd->bl.m].flag.nogo && battle_config.any_warp_GM_min_level > pc_isGM(sd) ) {
		clif_displaymessage(sd->fd,"Você não pode utilizar @go neste mapa.");
		return 0;
	}
	...
	if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data)) {
		...
		clif_displaymessage(fd, " 0=Prontera           1=Morocc         2=Geffen");
		...
		clif_displaymessage(fd, "12=Sala VIP");
	/*
...
ACMD_FUNC(storeall)
{
	...
	if (agit_flag == 1) {
		clif_displaymessage(fd, "Você não pode usar @storeall durante o período de guerras.");
		return -1;
	}
	...
	clif_displaymessage(fd, "Todos os itens foram depositados.");
	return 0;
}
...
ACMD_FUNC(autotrade)
{
	...
	if( map[sd->bl.m].flag.autotrade != battle_config.autotrade_mapflag ) {
		clif_displaymessage(fd, "Autotrade não é permitido neste mapa.");
		return -1;
	}

	if( pc_isdead(sd) ) {
		clif_displaymessage(fd, "Você precisa estar vivo para usar este comando.");
		return -1;
	}
	...
	if (!message || !*message || sscanf(message, "%d", &horas) < 1) {
		clif_displaymessage(fd, "Por favor, digite uma hora válida. Uso @at/@autotrade <horas>.");
		return -1;
	}
	
	if( pc_isGM(sd) < 1 && horas > 48 ) {
		clif_displaymessage(fd, "O limite máximo para membros comuns é de 48 horas!");
		return -1;
	}
	else if( pc_isGM(sd) == 1 ) {
		if ( ( horas > 168 && horas < 999 ) || horas > 999 ) {
			clif_displaymessage(fd, "O limite máximo para membros VIP é de 168 horas (7 dias)!");
			return -1;
		}
		if ( horas == 999 ) {
			if ( pc_readaccountreg(sd,"#autotrade_vip") > 3 ) {
				clif_displaymessage(fd, "Você só pode usar o autotrade especial (duração 1 mês) por três vezes ao ano!");
				return -1;
			}
			else {
				pc_setaccountreg(sd, "autotrade_vip", (pc_readaccountreg(sd,"#autotrade_vip") + 1));
				horas = 0;
			}
		}
	}
	...
	if( message[0] )
	{
		...
		status_change_start(&sd->bl, SC_AUTOTRADE, 10000, 0, 0, 0, 0, horas * 3600000, 0);
	}
...
ACMD_FUNC(changegm)
{
	...
	{
		clif_displaymessage(fd, "Você não pode usar este comando neste mapa.");
		return -1;
	}
	
	if (agit_flag == 1) {
		clif_displaymessage(fd, "Você não pode mudar o líder do seu clã durante o periodo de guerras.");
		return -1;
	}

	if( !message[0] )
	{
		clif_displaymessage(fd, "Uso do comando: @changegm <nome de membro do clã>");
		return -1;
	}
	
	if((pl_sd=map_nick2sd((char *) message)) == NULL || pl_sd->status.guild_id != sd->status.guild_id) {
		clif_displaymessage(fd, "O jogador escolhido deve estar online e ser um membro de seu clã.");
		return -1;
	}
...
ACMD_FUNC(changeleader)
{
	...
	if( !message[0] )
	{
		clif_displaymessage(fd, "Uso do comando: @changeleader <nome de membro do grupo>");
		return -1;
	}
...
ACMD_FUNC(autolootitem)
{
	...
	case 1:
		...
		if (i != AUTOLOOTITEM_SIZE) {
			clif_displaymessage(fd, "Este item já está no autoloot.");
			return -1;
		}
		ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] == 0);
		if (i == AUTOLOOTITEM_SIZE) {
			clif_displaymessage(fd, "Sua lista está cheia! Para remover algum item digite @autoloot -<nome ou ID do item>.");
			return -1;
		}
	...
	case 2:
		ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] == item_data->nameid);
		if (i == AUTOLOOTITEM_SIZE) {
			clif_displaymessage(fd, "Você não está usando autoloot neste item.");
			return -1;
		}
		sd->state.autolootid[i] = 0;
		sprintf(atcmd_output, "Removido item: '%s'/'%s' {%d} da lista de autoloot.", item_data->name, item_data->jname, item_data->nameid);
		clif_displaymessage(fd, atcmd_output);
		break;
	case 3:
		sprintf(atcmd_output, "Você tem %d itens em sua lista de autoloot.", AUTOLOOTITEM_SIZE);
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(fd, "Para adicionar a lista, use \"@alootid +<nome ou ID do item>\". Para remover digite \"@alootid -<nome ou ID do item>\".");
		clif_displaymessage(fd, "\"@alootid reset\" irá limpar a lista de itens.");
		ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] != 0);
		if (i == AUTOLOOTITEM_SIZE) {
			clif_displaymessage(fd, "Sua lista de itens está vazia.");
		} else {
			clif_displaymessage(fd, "Itens na sua lista de autoloot:");
	...
	case 4:
		...
		clif_displaymessage(fd, "Sua lista de autoloot foi resetada.");
		break;
	}
	return 0;
}
...
ACMD_FUNC(refresh)
{
	...
	if(!atcmd_checktime(sd)) {
		clif_displaymessage(fd,"Este comando possui um delay de 30 segundos.");
		return 0;
	}
	atcmd_savetime(sd);
...
ACMD_FUNC(identifyall)
{
	...
	//clif_displaymessage(fd,(indentify) ? "Todos os itens foram" : "Any item was identified");
    return 0;
}
...
ACMD_FUNC(mobinfo)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, insira o nome do monstro/ID (uso: @mobinfo <nome_do_monstro/id>).");
		return -1;
	}
	...
	for (k = 0; k < count; k++) {
		...
		sprintf(atcmd_output, " ATK:%d~%d  Alcance:%d~%d~%d  Tamanho:%s  Raça: %s  Elemento: %s (Nv:%d)",
		...
		if (j == 0)
			clif_displaymessage(fd, "Este monstro não dropa itens.");
		else if (j % 3 != 0)
			clif_displaymessage(fd, atcmd_output);
		// mvp
		if (mob->mexp) {
			...
			if (j == 0)
				clif_displaymessage(fd, "Este MVP não possui prêmios.");
			else
				clif_displaymessage(fd, atcmd_output);
		}
	}
...
ACMD_FUNC(iteminfo)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->maxchance == -1)
			...
			strcpy(atcmd_output, " - Nenhum monstro dropa este item.");
		clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(whodrops)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			strcpy(atcmd_output, " - Item não é dropado por monstros.");
			clif_displaymessage(fd, atcmd_output);
		} else {
			sprintf(atcmd_output, "- Monstros comuns com maior chance de dropar o item (apenas os %d maiores serão exibidos):", MAX_SEARCH);
			clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(whereis)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, insira o nome do monstro/ID (uso: @whereis <nome_do_monstro ou ID>).");
		return -1;
	}
	...
	for (k = 0; k < count; k++) {
		mob = mob_array[k];
		snprintf(atcmd_output, sizeof atcmd_output, "%s spawn em:", mob->jname);
		clif_displaymessage(fd, atcmd_output);
		...
		if (i == 0)
			clif_displaymessage(fd, "Este monstro não possui spawn regular.");
	}
...
ACMD_FUNC(rates)
{
	...
	snprintf(buf, CHAT_SIZE_MAX, "Taxa de Experiência: Base %.2fx / Job %.2fx",
		battle_config.base_exp_rate/100., battle_config.job_exp_rate/100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, "Taxa de Drop Comum: Comum %.2fx / Cura %.2fx / Usável %.2fx / Equipamento %.2fx / Carta %.2fx",
	...
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, "Taxa de Drop MvP: Comum %.2fx / Cura %.2fx / Usável %.2fx / Equipamento %.2fx / Carta %.2fx",
	...
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, "Outras Taxas de Drop: MvP %.2fx / Baseado em Cartas %.2fx / Tesouro %.2fx",
...
ACMD_FUNC(showexp)
{
	if (sd->state.showexp) {
		sd->state.showexp = 0;
		clif_displaymessage(fd, "Experiência ganha não será exibida.");
		return 0;
	}

	sd->state.showexp = 1;
	clif_displaymessage(fd, "Experiência ganha será exibida.");
	return 0;
}
...
ACMD_FUNC(showdelay)
{
	if (sd->state.showdelay) {
		...
		clif_displaymessage(fd, "Mensagens de falha no reuso de habilidades não serão exibidas.");
		return 0;
	}
	
	sd->state.showdelay = 1;
	clif_displaymessage(fd, "Mensagens de falha no reuso de habilidades serão exibidas.");
	return 0;
}
...
ACMD_FUNC(ksprotection)
{
	...
	if( sd->state.noks ) {
		sd->state.noks = 0;
		sprintf(atcmd_output, ".:: Proteção KS Desativada ::.");
	}
	else
	{
		...
		{ // Default is Party
			sd->state.noks = 2;
			sprintf(atcmd_output, ".:: Proteção KS Ativada - Modo: Grupo ::.");
		}
		else if( !strcmpi(message, "self") )
		{
			sd->state.noks = 1;
			sprintf(atcmd_output, ".:: Proteção KS Ativada - Modo: Jogador ::.");
		}
		else if( !strcmpi(message, "guild") )
		{
			sd->state.noks = 3;
			sprintf(atcmd_output, ".:: Proteção KS Ativada - Modo: Clã ::.");
		}
		else
		...
		{ 
			...
			for (j = 0; j < pl_sd->vend_num; j++) { 
				...
				// Se o item é o mesmo, ele vai mostrar isso? 
					sprintf(output, "Vendedor: %s | Quantia: %d | Preço: %d | Shop: %s | Coord: %d,%d", pl_sd->status.name, pl_sd->vending[j].amount, pl_sd->vending[j].value,pl_sd->message,pl_sd->bl.x,pl_sd->bl.y); 
...
ACMD_FUNC(displaymessage) //custom
{
/*	struct map_session_data *pl_sd;
	nullpo_retr(-1, sd);
	char text[255];
	...
	if (!message || !*message || sscanf(message, "%255s", text) < 1) { 
		clif_displaymessage(fd, "Uso: @displaymessage <mensagem>"); 
		return -1; 
	} 
	
	snprintf(atcmd_output, sizeof atcmd_output, "%s", text);
*/
	clif_displaymessage(fd, "Olá");
...
ACMD_FUNC(trigger)
{
    ...
    sprintf(atcmd_output,"Voce engatilhou a label %s.",message);
    return 0;
}
...
void account_info(const int fd, struct map_session_data *sd, int account_id) {
	...
	sprintf(atcmd_output, "-- Conta %d --", account_id);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output, "Usuário: %s | Nv. GM: %d | Estado: %d", userid, level, state);
	clif_displaymessage(fd, atcmd_output);
	if (level < pc_isGM(sd)) {
		sprintf(atcmd_output, "Senha: %s", user_pass);
		clif_displaymessage(fd, atcmd_output);
	}
	
	sprintf(atcmd_output, "E-mail da conta: %s.", email);
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output, "Último IP: %s (%s)", last_ip, geoip_getcountry(str2ip(last_ip)));
	clif_displaymessage(fd, atcmd_output);
	sprintf(atcmd_output, "O usuário logou %d vezes, o último login foi em %s.", logincount, lastlogin);
	clif_displaymessage(fd, atcmd_output);
	
	clif_displaymessage(fd, "-- Detalhes do Personagem --");
	...
	if ( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `job_level`, `online` FROM `char` WHERE `account_id` = '%d' ORDER BY `char_num`", account_id) )
		...
		clif_displaymessage(fd, "Esta conta não possui personagens.");
	else {
...
ACMD_FUNC(accountinfo)
{
	...
	if (account_id < START_ACCOUNT_NUM) {
		...
		{
			clif_displaymessage(fd, "Query Error on @accinfo.");
			Sql_FreeResult(mmysql_handle);
			return -1;
		} else if ( Sql_NumRows(mmysql_handle) == 0 ) {
			clif_displaymessage(fd, "Invalid Account ID or Character Name");
			Sql_FreeResult(mmysql_handle);
			return -1;
		} else {
...
void atcommand_basecommands(void) {
	...
	AtCommandInfo atcommand_base[] = {
		...
		{ "where",             20,99,     atcommand_where },
		{ "goto",              20,20,     atcommand_jumpto }, // + /shift
		{ "jump",              20,40,     atcommand_jump },
...
#include "harmony_atcommanddef.inc"
		...
		{ "petrename",          1,99,     atcommand_petrename },
...
//		{ "gmotd",             20,20,     atcommand_gmotd },
		...
		{ "autoloot",           0,10,     atcommand_autoloot },
		{ "alootid",            1,10,     atcommand_autolootitem },
		{ "monsterinfo",        0,99,     atcommand_mobinfo },
		{ "exp",                1,99,     atcommand_exp },
		{ "adopt",             40,40,     atcommand_adopt },
		{ "version",           40,99,     atcommand_version },
		{ "mutearea",          99,99,     atcommand_mutearea },
		{ "rates",              0,99,     atcommand_rates },
		{ "iteminfo",           0,99,     atcommand_iteminfo },
		{ "whodrops",           1,99,     atcommand_whodrops },
		{ "whereis",            0,10,     atcommand_whereis },
		...
		{ "showexp",            1,10,     atcommand_showexp},
		{ "showzeny",           1,10,     atcommand_showzeny},
		{ "showdelay",          1,99,     atcommand_showdelay},
		{ "autotrade",          0,10,     atcommand_autotrade },
		{ "changegm",           1,10,     atcommand_changegm },
		{ "changeleader",       1,10,     atcommand_changeleader },
		{ "partyoption",       10,10,     atcommand_partyoption},
		{ "invite",             0,99,     atcommand_invite },
		{ "duel",               0,99,     atcommand_duel },
		{ "leave",              0,99,     atcommand_leave },
		{ "accept",             0,99,     atcommand_accept },
		{ "reject",             0,99,     atcommand_reject },
		{ "main",               0,99,     atcommand_main },
		{ "clone",             50,50,     atcommand_clone },
		{ "slaveclone",        50,50,     atcommand_clone },
		{ "evilclone",         50,50,     atcommand_clone },
		{ "tonpc",             40,40,     atcommand_tonpc },
		{ "commands",           0,99,     atcommand_commands },
		{ "noask",              0,99,     atcommand_noask },
		...
		{ "hominfo",           99,99,     atcommand_hominfo },
		{ "homstats",          99,99,     atcommand_homstats },
		{ "homshuffle",        60,60,     atcommand_homshuffle },
		{ "showmobs",          10,10,     atcommand_showmobs },
		{ "feelreset",         10,10,     atcommand_feelreset },
		{ "auction",           99,99,     atcommand_auction },
		{ "mail",              99,99,     atcommand_mail },
		{ "noks",               0,99,     atcommand_ksprotection },
		...
		{ "charcommands",      99,99,     atcommand_charcommands },
		{ "font",              99,99,     atcommand_font },
		//custom
		{ "identifyall",	   20,99,	  atcommand_identifyall },
		{ "whosell",            1,99,     atcommand_whosell },
		{ "displaymessage",	   99,99,	  atcommand_displaymessage }, 
		{ "trigger",           99,99,     atcommand_trigger },
		{ "away",               1,99,     atcommand_away },
		{ "aw",                 1,99,     atcommand_away },
		{ "vip",			    1,99,	  atcommand_vip },
		{ "accinfo",           99,100,    atcommand_accountinfo },
	};
	...
	if( (info = (AtCommandInfo*)strdb_get(atcommand_db, command+1)) == NULL || info->func == NULL || ( type && ((*atcmd_msg == atcommand_symbol && pc_isGM(sd) < info->level) || (*atcmd_msg == charcommand_symbol && pc_isGM(sd) < info->level2)) ) )
	{
		if( pc_isGM(sd) ) {
			if ( pc_isGM(sd) < 20 ) {
				...
				clif_disp_onlyself(sd, msg_txt(153), (int)strlen(msg_txt(153)));
			}
			else {
