...
int pc_reg_received(struct map_session_data *sd)
{
	...
	sd->ip = session[sd->fd]->client_addr;
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	{	//asa de mosca
		if( sd->status.inventory[n].expire_time == 0 )// && (!(pc_isGM(sd) >= 1 && (sd->status.inventory[n].nameid == 601))))
		{
...
int atcmd_checktime(struct map_session_data* sd)
{
    int diff;
    time_t timer;
    struct tm *t;
