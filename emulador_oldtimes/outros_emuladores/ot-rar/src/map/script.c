...
///********************************************
/// rentitem2 <item id>,<tempo>,<quantidade>
///********************************************
...
BUILDIN_FUNC(ipcheck)
{
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if(!pl_sd || !sd) return 0;
		if(type == 1)
		{
			if (pl_sd->ip == sd->ip &&
				sd->bl.m == pl_sd->bl.m &&
				pl_sd->status.account_id != sd->status.account_id)				// That will danied double ip just in a map of your server
			vals = 1;
		} else {
			if (pl_sd->ip == sd->ip &&
				pl_sd->status.account_id != sd->status.account_id)				// That will danied double ip in everymap of your server
			vals = 1;
		}
	}
	script_pushint(st,vals);
	return 0;
}
...
// <--- [zBuffer] List of mob control commands
	...
	// rentitem2
	BUILDIN_DEF(rentitem2,"vii"),
	
	// custom
	BUILDIN_DEF(ipcheck,"i"),			//[AnimaL Sina]
