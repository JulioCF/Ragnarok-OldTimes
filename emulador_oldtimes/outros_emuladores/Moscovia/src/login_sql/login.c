	...
	while( RFIFOREST(fd) >= 2 )
	{
		...
		case 0x272b:    // Set account_id to online [Wizputer]
			...
			add_online_user(id, RFIFOL(fd,2));
						// SQL Query, feita por eth0, que faz o seguinte: Uma account, ao passar pelo login server, leva um ban de
						// 60 segundos, que, se o player esperar na tela de login, tem um "timeout", para a seleção do char, e irá ser barrada
						// a seleção de char, então a pessoa não poderá fazer um "double login". Considere esta atualização como uma
						// 'proteção adicional', vendo que o bug de double login já foi corrigido.[MaxDragon]
						// Ative a correção tirando o // da linha seguinte.
			//Sql_Query(sql_handle, "update login set ban_until = unix_timestamp(now())+60 where account_id = %d",RFIFOL(fd,2));
			RFIFOSKIP(fd,6);
		break;
...
void login_auth_ok(struct login_session_data* sd)
{
	...
	if( sd->level > 0 )
		...
		ShowStatus("Connection of the account '%s' accepted.\n", sd->userid);
						...
						// 'proteção adicional', vendo que o bug de double login já foi corrigido.[MaxDragon]
						// Ative a correção tirando o // da linha seguinte.
	//Sql_Query(sql_handle, "update login set ban_until = unix_timestamp(now())+10 where userid = '%s'",esc_userid);
