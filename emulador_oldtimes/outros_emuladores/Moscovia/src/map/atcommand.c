...
int atcommand_send(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Como usar:");
		clif_displaymessage(fd, "	@send len <número hexadecimal do packet>");
		clif_displaymessage(fd, "	@send <número hexadecimal do packet> {<valor>}*");
	...
	if (!message || !*message ||
		(sscanf(message, "%15s %d %d", map_name, &x, &y) < 3 &&
		 sscanf(message, "%15[^,],%d,%d", map_name, &x, &y) < 1)) {
			...
			clif_displaymessage(fd, "Por favor, entre com um mapa (Como usar: @warp/@rura/@mapmove <nome do mapa> <x> <y>).");
			return -1;
	}
...
int atcommand_where(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @where <nome do jogador>).");
		return -1;
	}
...
int atcommand_jumpto(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @jumpto/@warpto/@goto <nome do jogador/id>).");
		return -1;
	}
...
int atcommand_whomap(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		if (pl_GM_level > 0)
			sprintf(atcmd_output, "Nome: %s (GM:%d) | Party: '%s' | Guild: '%s'", pl_sd->status.name, pl_GM_level, temp0, temp1);
		else
			sprintf(atcmd_output, "Nome: %s | Party: '%s' | Guild: '%s'", pl_sd->status.name, temp0, temp1);
		clif_displaymessage(fd, atcmd_output);
		count++;
	}
	mapit_free(iter);
...
int atcommand_speed(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &speed) < 1) {
		sprintf(atcmd_output, "Por favor, digite um valor permitido (Como usar: @speed <%d-%d>).", MIN_WALK_SPEED, MAX_WALK_SPEED);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_guildstorage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	clif_displaymessage(fd, "Armazém da guild aberto.");
	return 0;
}
...
int atcommand_option(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &param1, &param2, &param3) < 1 || param1 < 0 || param2 < 0 || param3 < 0) {
		clif_displaymessage(fd, "Por favor, entre com no mínimo uma opção (Como usar: @option <param1:0+> <param2:0+> <param3:0+>).");
		return -1;
	}
...
int atcommand_jobchange(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1)
	{
		...
		if (!found) {
			clif_displaymessage(fd, "Por favor, entre com o ID de uma classe (Como usar: @job/@jobchange <nome da classe/ID>).");
			...
			clif_displaymessage(fd, "   3 Arqueiro           10 Ferreiro               17 Arruaceiro          24 Gunslinger");
			clif_displaymessage(fd, "   4 Noviço             11 Caçador                18 Alquimista          25 Ninja");
			clif_displaymessage(fd, "   5 Mercador           12 Mercenário             19 Bardo               26 Natal");
			clif_displaymessage(fd, "   6 Gatuno             13 Cavaleiro (Peco)       20 Odalisca");
			clif_displaymessage(fd, "4001 Aprendiz T.      4008 Lorde                4015 Paladino          4022 Peco Paladin");
			...
			clif_displaymessage(fd, "4005 Noviço T.        4012 Atirador Elite (Não utilize o de)    4019 Criador");
			clif_displaymessage(fd, "4006 Mercador T.      4013 Algoz                4020 Menestrel");
			clif_displaymessage(fd, "4007 Gatuno T.        4014 Peco Lord Knight     4021 Gypsy");
			clif_displaymessage(fd, "4023 Bebê Aprendiz    4030 Bebê Cavaleiro       4037 Bebê Templário    4044 Bebê Templário (Peco) ");
			...
			clif_displaymessage(fd, "4026 Bebê Arqueiro    4033 Bebê Ferreiro        4040 Bebê Arruaceiro   4047 Mestre Taekwon");
			clif_displaymessage(fd, "4027 Baby Acolyte     4034 Bebê Caçador         4041 Bebê Alquimista   4048 N/A");
			clif_displaymessage(fd, "4028 Bebê Mercador    4035 Bebê Mercenário      4042 Bebê Bardo        4049 Espiritualista");
			clif_displaymessage(fd, "4029 Bebê Gatuno      4036 Bebê Cavaleiro Peco  4043 Bebê Dançarina");
			clif_displaymessage(fd, "[upper]: -1 (padrão) para automaticamente determinar o 'nível', 0 para forçar uma classe normal, 1 para forçar uma classe superior.");
			return -1;
		}
	}
	...
	if (pcdb_checkid(job))
	{
		...
		clif_displaymessage(fd, "Por favor, entre com um ID de classe válido (Como usar: @job/@jobchange <nome da classe/ID>).");
		clif_displaymessage(fd, "   0 Aprendiz            7 Cavaleiro              14 Templário           21 Templário (Peco)");
		clif_displaymessage(fd, "   1 Espadachim          8 Sacerdote              15 Monge               22 Casamento");
		clif_displaymessage(fd, "   2 Mago                9 Bruxo                  16 Sábio               23 Super Aprendiz");
		clif_displaymessage(fd, "   3 Arqueiro           10 Ferreiro               17 Arruaceiro          24 Gunslinger");
...
int atcommand_kill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @kill <nome do jogador/id>).");
		return -1;
	}
...
int atcommand_kami(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(*(command + 5) != 'c' && *(command + 5) != 'C') {
		...
		if (!message || !*message) {
			clif_displaymessage(fd, "Por favor, entre uma mensagem (Como usar: @kami <mensagem>).");
			return -1;
		}
		...
		if(!message || !*message || (sscanf(message, "%lx %199[^\n]", &color, atcmd_output) < 2)) {
			clif_displaymessage(fd, "Por favor, entre com a cor e a mensagem (usage: @kamic <cor> <mensagem>).");
			return -1;
		}
...
int atcommand_item(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com um nome de item/ID e a quantidade (Como usar: @item <nome do item/ID> [quantidade]).");
		return -1;
	}
...
int atcommand_item2(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com todas as informações (Como usar: @item2 <nome do item/ID> <quantidade>");
		clif_displaymessage(fd, "  <Flag de Identificação [1=S,0=N]> <refine> <atributos> <Carta1> <Carta2> <Carta3> <Carta4>).");
		return -1;
	}
...
int atcommand_baselevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, "Por favor, entre com um ajuste de níveis (Como usar: @lvup/@blevel/@baselvlup <número de níveis>).");
		return -1;
	}
...
int atcommand_joblevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, "Por favor, entre com um ajuste de níveis (Como usar: @joblvup/@jlevel/@joblvlup <número de níveis>).");
		return -1;
	}
	if (level > 0) {
...
int atcommand_gm(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", password) < 1) {
		clif_displaymessage(fd, "Por favor, entre com uma senha (Como usar: @gm <senha>).");
		return -1;
	}
...
int atcommand_model(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &hair_style, &hair_color, &cloth_color) < 1) {
		sprintf(atcmd_output, "Por favor, entre com no mínimo um valor (Como usar: @model <ID do cabelo: %d-%d> <Cor do cabelo: %d-%d> <Cor da roupa: %d-%d>).",
...
int atcommand_dye(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &cloth_color) < 1) {
		sprintf(atcmd_output, "Por favor, entre com a cor da roupa (Como usar: @dye/@ccolor <Cor da roupa: %d-%d>).", MIN_CLOTH_COLOR, MAX_CLOTH_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_hair_style(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &hair_style) < 1) {
		sprintf(atcmd_output, "Por favor, entre com um estilo de cabelo (Como usar: @hairstyle/@hstyle <ID do cabelo: %d-%d>).", MIN_HAIR_STYLE, MAX_HAIR_STYLE);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_hair_color(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &hair_color) < 1) {
		sprintf(atcmd_output, "Por favor, entre com uma cor de cabelo (Como usar: @haircolor/@hcolor <cor do cabelo: %d-%d>).", MIN_HAIR_COLOR, MAX_HAIR_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_go(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if( map[sd->bl.m].flag.nogo && battle_config.any_warp_GM_min_level > pc_isGM(sd) ) {
		clif_displaymessage(sd->fd,"Você não pode usar @go para ir a este mapa.");
		return 0;
	}
...
int atcommand_monstersmall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Dê um nome/ID de um monstro.");
		return -1;
	}
...
int atcommand_refine(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &position, &refine) < 2) {
		clif_displaymessage(fd, "Por favor, entre a posição e a quantidade (Como usar: @refine <posição do equip> <+/- quantidade>).");
		...
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, "%d: Capa", EQI_GARMENT);
		clif_displaymessage(fd, atcmd_output);
...
int atcommand_produce(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com no mínimo um nome de equipamento/ID (Como usar: @produce <nome do equipamento/ID> <elemento> <# de very's>).");
		return -1;
	}
...
int atcommand_memo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		int i;
		clif_displaymessage(sd->fd,  "Sua posição de memo atual é:");
	...
	{
		sprintf(atcmd_output, "Por favor, utilize uma localização válida (Como usar: @memo <posição memorizada:%d-%d>).", 0, MAX_MEMOPOINTS-1);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_displaystatus(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (i = sscanf(message, "%d %d", &type, &flag)) < 1) {
		clif_displaymessage(fd, "Por favor, entre com uma bandeira/status (Como usar: @displaystatus <tipo de status> <bandeira>).");
		return -1;
	}
	if (i == 1) flag = 1;
...
int atcommand_statuspoint(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(fd, "Por favor, entre com um número (Como usar: @stpoint <número de pontos>).");
		return -1;
	}
...
int atcommand_skillpoint(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(fd, "Por favor, entre com um número (Como usar: @skpoint <número de pontos>).");
		return -1;
	}
...
int atcommand_zeny(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (zeny = atoi(message)) == 0) {
		clif_displaymessage(fd, "Por favor, entre com uma quantidade (Como usar: @zeny <quantidade>).");
		return -1;
	}
...
int atcommand_param(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &value) < 1 || value == 0) {
		sprintf(atcmd_output, "Por favor, entre com um valor válido (Como usar: @str,@agi,@vit,@int,@dex,@luk <+/- quantidade>).");
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_guildlevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &level) < 1 || level == 0) {
		clif_displaymessage(fd, "Por favor, entre um nível válido (Como usar: @guildlvup/@guildlvlup <# de níveis>).");
		return -1;
	}
...
int atcommand_makeegg(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o ID do ovo/nome do monstro (Como usar: @makeegg <pet>).");
		return -1;
	}
...
int atcommand_petfriendly(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (friendly = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um valor válido (Como usar: @petfriendly <0-1000>).");
		return -1;
	}
...
int atcommand_pethungry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (hungry = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre um número válido (Como usar: @pethungry <0-100>).");
		return -1;
	}
...
int atcommand_recall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @recall <nome do jogador/ID>).");
		return -1;
	}
	...
	if (sd->bl.m >= 0 && map[sd->bl.m].flag.nowarpto && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
		clif_displaymessage(fd, "Você não é autorizado a chamar um player para o mapa onde você está.");
		return -1;
	}
	if (pl_sd->bl.m >= 0 && map[pl_sd->bl.m].flag.nowarp && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
		clif_displaymessage(fd, "Você não é autorizado a remover o jogador do mapa onde ele está.");
		return -1;
	}
...
int atcommand_char_block(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: @charblock/@block <nome>).");
		return -1;
	}
...
int atcommand_char_ban(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%s %23[^\n]", atcmd_output, atcmd_player_name) < 2) {
		clif_displaymessage(fd, "Por favor, entre com o tempo de ban e o nome do jogador (Como usar: @charban/@ban/@banish/@charbanish <tempo> <nome>).");
		return -1;
	}
...
int atcommand_char_unblock(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre o nome de um jogador (Como usar: @charunblock <nome do jogador>).");
		return -1;
	}
...
int atcommand_char_unban(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre o nome de um jogador (Como usar: @charunban <nome do jogador>).");
		return -1;
	}
...
int atcommand_kick(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre o nome de um jogador (Como usar: @kick <nome do jogador/ID>).");
		return -1;
	}
...
int atcommand_questskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (skill_id = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre um número de skill de quest (Como usar: @questskill <#:0+>).");
		return -1;
	}
...
int atcommand_lostskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (skill_id = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um número de skill (Como usar: @lostskill <#:0+>).");
		return -1;
	}
...
int atcommand_spiritball(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (number = atoi(message)) < 0) {
		clif_displaymessage(fd, "Por favor, entre com um número de esferas espirítuais (Como usar: @spiritball <número: 0-1000>).");
		return -1;
	}
...
int atcommand_party(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party) < 1) {
		clif_displaymessage(fd, "Por favor, entre o nome de um grupo (Como usar: @party <nome do grupo>).");
		return -1;
	}
...
int atcommand_guild(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de uma guild (Como usar: @guild <nome da guild>).");
		return -1;
	}
...
int atcommand_idsearch(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s", item_name) < 0) {
		clif_displaymessage(fd, "Por favor, entre com no mínimo uma parte do nome do item. DICA: Use _ como espaço.(Como usar: @idsearch <nome_do_item>).");
		return -1;
	}
...
int atcommand_recallall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->bl.m >= 0 && map[sd->bl.m].flag.nowarpto && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
		clif_displaymessage(fd, "Você não é autorizado a chamar alguém para seu mapa atual.");
		return -1;
	}
	...
	if (count) {
		sprintf(atcmd_output, "Pelo motivo de que você não pode dar warp para alguns mapas, %d jogador(es) não foram chamados.", count);
		clif_displaymessage(fd, atcmd_output);
	}
...
int atcommand_guildrecall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de uma guild/ID (Como usar: @guildrecall <nome da guild/ID>).");
		return -1;
	}
...
int atcommand_partyrecall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre o nome/ID de um grupo (Como usar: @partyrecall <nome do grupo/ID>).");
		return -1;
	}
...
int atcommand_mapinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (list < 0 || list > 3) {
		clif_displaymessage(fd, "Por favor, entre com no mínimo um valor válido de listas (Como usar: @mapinfo <0-3> [mapa]).");
		return -1;
	}
	...
	clif_displaymessage(fd, "------ Informação dos Mapas ------");
	...
	case 1:
		...
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
			if (pl_sd->mapindex == m_index) {
				sprintf(atcmd_output, "Jogadores '%s' (sessão #%d) | Localização: %d,%d",
	...
	case 3:
		...
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
			    ...
			    cd->usersd[0] == pl_sd)
			{
				...
				clif_displaymessage(fd, atcmd_output);
				sprintf(atcmd_output, "   Usuários: %d/%d | Senha: %s | Publico: %s",
	...
	default: // normally impossible to arrive here
		clif_displaymessage(fd, "Por favor, entre um valor válido de listagem (Como usar: @mapinfo <0-3> [mapa]).");
		return -1;
		break;
	}
...
int atcommand_guildspy(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!enable_spy)
	{
		clif_displaymessage(fd, "O MapServer está com o suporte a espião desativado.");
		return -1;
	}
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome da guild/ID (Como usar: @guildspy <nome da guild/ID>).");
		return -1;
	}
...
int atcommand_partyspy(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome da party/ID (Como usar: @partyspy <nome da party/ID>).");
		return -1;
	}
...
int atcommand_nuke(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @nuke <nome do jogador>).");
		return -1;
	}
...
int atcommand_tonpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", npcname) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um NPC (Como usar: @tonpc <nome do NPC>).");
		return -1;
	}
...
int atcommand_shownpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", NPCname) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um NPC (Como usar: @enablenpc <nome do NPC>).");
		return -1;
	}
...
int atcommand_hidenpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", NPCname) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um NPC (Como usar: @hidenpc <nome do NPC>).");
		return -1;
	}
...
int atcommand_loadnpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um arquivo de script (Como usar: @loadnpc <nome do arquivo>).");
		return -1;
	}
...
int atcommand_unloadnpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", NPCname) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um NPC (Como usar: @npcoff <nome do NPC>).");
		return -1;
	}
...
int atcommand_jail(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @jail <nome do char>).");
		return -1;
	}
...
int atcommand_unjail(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @unjail/@discharge <nome do char>).");
		return -1;
	}
...
int atcommand_jailtime(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->sc.data[SC_JAILED]->val1 == INT_MAX) {
		clif_displaymessage(fd, "Você foi preso para sempre.");
		return 0;
	}

	if (sd->sc.data[SC_JAILED]->val1 <= 0) { // Was not jailed with @jailfor (maybe @jail? or warped there? or got recalled?)
		clif_displaymessage(fd, "Você foi preso até uma data indefinida.");
		return -1;
	}

	//Get remaining jail time
	get_jail_time(sd->sc.data[SC_JAILED]->val1,&year,&month,&day,&hour,&minute);
	sprintf(atcmd_output,msg_txt(402),"Ainda faltam",year,month,day,hour,minute); // You will remain in jail for %d years, %d months, %d days, %d hours and %d minutes
...
int atcommand_disguise(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre o nome/ID de um monstro/NPC (Como usar: @disguise <nome ou ID do monstro/NPC>).");
		return -1;
	}
...
int atcommand_disguiseall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre o nome/ID de um monstro/NPC (Como usar: @disguiseall <nome ou ID do monstro/NPC>).");
		return -1;
	}
...
int atcommand_broadcast(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com uma mensagem (Como usar: @broadcast <mensagem>).");
		return -1;
	}
...
int atcommand_localbroadcast(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com uma mensagem (Como usar: @localbroadcast <mensagem>).");
		return -1;
	}
...
int atcommand_email(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s %99s", actual_email, new_email) < 2) {
		clif_displaymessage(fd, "Por favor, entre 2 emails (Como usar: @email <email@atual> <email@novo>).");
		return -1;
	}
...
int atcommand_effect(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &type) < 1) {
		clif_displaymessage(fd, "Por favor, entre com um número de efeito (Como usar: @effect <número do efeito>).");
		return -1;
	}
...
int atcommand_npcmove(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %23[^\n]", &x, &y, atcmd_player_name) < 3) {
		clif_displaymessage(fd, "Como usar: @npcmove <X> <Y> <nome do npc>");
		return -1;
	}
...
int atcommand_addwarp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%31s %d %d", mapname, &x, &y) < 3) {
		clif_displaymessage(fd, "Como usar: @addwarp <nome do mapa> <X> <Y>.");
		return -1;
	}
	...
	sprintf(atcmd_output, "Novo portal '%s' foi criado.", nd->exname);
	clif_displaymessage(fd, atcmd_output);
	return 0;
}
...
int atcommand_storeall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->state.storage_flag != 1)
  	{	//Open storage.
		...
		case 2: //Try again
			clif_displaymessage(fd, "executar este comando novamente..");
			return 0;
		case 1: //Failure
			clif_displaymessage(fd, "Você não pôde abrir o armazém neste momento.");
			return -1;
		}
	}
	for (i = 0; i < MAX_INVENTORY; i++) {
...
int atcommand_skillid(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message)
	{
		clif_displaymessage(fd, "Por favor, entre com o nome de um skill (Como usar: @skillid <nome do skill>).");
		return -1;
	}
...
int atcommand_useskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(!message || !*message || sscanf(message, "%d %d %23[^\n]", &skillnum, &skilllv, target) != 3) {
		clif_displaymessage(fd, "Como usar: @useskill <número da skill> <nível da skill> <alvo>");
		return -1;
	}
...
int atcommand_displayskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Como usar: @displayskill <número da skill> {<nível da skill>}>");
		return -1;
	}
...
int atcommand_skilltree(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(!message || !*message || sscanf(message, "%d %23[^\r\n]", &skillnum, target) != 2) {
		clif_displaymessage(fd, "Como usar: @skilltree <número da skill> <alvo>");
		return -1;
	}
	...
	sprintf(atcmd_output, "O jogador está usando a árvore de habilidades %s(%d pontos de base)",
	...
	if (skillidx == -1) {
		sprintf(atcmd_output, "Eu acredito que este jogador não possa usar este skill");
		clif_displaymessage(fd, atcmd_output);
		return 0;
	}
	...
	for(j=0;j<5;j++)
		...
		{
			sprintf(atcmd_output, "o jogador precisa ter o nível %d da skill %s", ent->need[j].lv, skill_db[ent->need[j].id].desc);
			clif_displaymessage(fd, atcmd_output);
			meets = 0;
		}

		if (meets == 1) {
			sprintf(atcmd_output, "Eu acredito que o jogador não tenha todos os requerimentos para uso desta habilidade");
			clif_displaymessage(fd, atcmd_output);
		}
...
int atcommand_marry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
  ...
  if (!message || !*message || sscanf(message, "%23[^,], %23[^\r\n]", player1, player2) != 2) {
	clif_displaymessage(fd, "Como usar: @marry <jogador1>,<jogador2>");
	return -1;
  }

  if((pl_sd1=map_nick2sd((char *) player1)) == NULL) {
	sprintf(player2, "Não foi possível encontrar o jogador '%s' online", player1);
	clif_displaymessage(fd, player2);
	return -1;
  }

  if((pl_sd2=map_nick2sd((char *) player2)) == NULL) {
	sprintf(player1, "Não foi possível encontrar o jogador '%s' online", player2);
	clif_displaymessage(fd, player1);
	return -1;
  }

  if (pc_marriage(pl_sd1, pl_sd2) == 0) {
	clif_displaymessage(fd, "Eles estão casados, deseje-lhes felicidades!");
	...
	clif_displaymessage(fd, "Os dois não puderam casar pois um deles é bebê ou está atualmente casado.");
	return -1;
}
...
int atcommand_divorce(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
  ...
  if (!message || !*message || sscanf(message, "%23[^\r\n]", atcmd_player_name) != 1) {
	clif_displaymessage(fd, "Como usar: @divorce <jogador>.");
	return -1;
  }
	...
	if (pc_divorce(pl_sd) != 0) {
		sprintf(atcmd_output, "O divórcio não pode ser feito.. Não foi possível encontrar '%s' ou seu(ua) parceira online.", atcmd_player_name);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	
	sprintf(atcmd_output, "'%s' e seu(ua) parceira foram divorciados.", atcmd_player_name);
	clif_displaymessage(fd, atcmd_output);
	return 0;
}
...
int atcommand_changelook(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if((i = sscanf(message, "%d %d", &j, &k)) < 1) {
		clif_displaymessage(fd, "Como usar: @changelook [<posição>] <id de visialização> -- [] = opcional");
		clif_displaymessage(fd, "Posição: 1-Topo 2-Meio 3-Embaixo 4-Arma 5-Escudo");
		return -1;
	} else if (i == 2) {
		if (j < 1) j = 1;
...
int atcommand_changegm(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Você precisa ser o dono da guild para utilizar este comando.");
		return -1;
	}
	if (strlen(message)==0)
	{
		clif_displaymessage(fd, "Como utilizar o comando: @changegm <nome do membro da guild>");
		return -1;
	}
	
	if((pl_sd=map_nick2sd((char *) message)) == NULL || pl_sd->status.guild_id != sd->status.guild_id) {
		clif_displaymessage(fd, "O personagem deve ser membro da guild e estar online.");
		return -1;
	}
...
int atcommand_changeleader(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (strlen(message)==0)
	{
		clif_displaymessage(fd, "Como utilizar o comando: @changeleader <nome do membro da party>");
		return -1;
	}
...
int atcommand_partyoption(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Como utilizar o comando: @partyoption <divisão de itens: sim/não> <distribuição de items: sim/não>");
		return -1;
	}
...
int atcommand_autoloot(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->state.autoloot) {
		snprintf(atcmd_output, sizeof atcmd_output, "Recolhimento automático de items com chance de drop de %0.02f%% e abaixo ativado.",((double)sd->state.autoloot)/100.);
		clif_displaymessage(fd, atcmd_output);
	}else
		clif_displaymessage(fd, "Recolhimento automático de items desativado.");
	return 0;
}
...
int atcommand_rain(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.rain) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Foi feito chover.");
	}
	return 0;
}
...
int atcommand_snow(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.snow) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Foi feito nevar.");
	}
...
int atcommand_sakura(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.sakura) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Folhas de cerejeira não irão mais cair.");
	} else {
		map[sd->bl.m].flag.sakura=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Folhas de cerejeira estão caindo.");
	}
	return 0;
}
...
int atcommand_clouds(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.clouds) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "As nuvems desapareceram.");
	} else {
		map[sd->bl.m].flag.clouds=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Nuvems apareceram.");
	}
...
int atcommand_clouds2(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.clouds2) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "As nuvems alternativas desapareceram.");
	} else {
		map[sd->bl.m].flag.clouds2=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Nuvems alternativas apareceram.");
	}
...
int atcommand_fog(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.fog) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "A neblina cobre tudo.");
	}
		return 0;
}
...
int atcommand_leaves(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.leaves) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Folhas não caem mais.");
	} else {
		map[sd->bl.m].flag.leaves=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Folhas caindo.");
	}
...
int atcommand_fireworks(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.fireworks) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Os fogos de artifício acabaram.");
	} else {
		map[sd->bl.m].flag.fireworks=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "Fogos de artifício são lançados.");
	}
...
int atcommand_sound(const int fd, struct map_session_data *sd, const char *command, const char *message)
{
	...
	memset(sound_file, '\0', sizeof(sound_file));
		...
		if(!message || !*message || sscanf(message, "%99[^\n]", sound_file) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um arquivo de som. (Como usar: @sound <nome do arquivo>)");
		return -1;
	}
...
int atcommand_mobsearch(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mob_name) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um monstro (Como usar: @mobsearch <nome do monstro>).");
		return -1;
	}
...
//				strcpy(mob_name,mob_db(mob_id)->name);	// --en--
	...
	snprintf(atcmd_output, sizeof atcmd_output, "Procura de monstro... %s %s", mob_name, mapindex_id2name(sd->mapindex));
	clif_displaymessage(fd, atcmd_output);
...
int atcommand_cleanmap(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	map_foreachinarea(atcommand_cleanmap_sub, sd->bl.m,
		...
		sd->bl.x+AREA_SIZE*2, sd->bl.y+AREA_SIZE*2,
		BL_ITEM);
	clif_displaymessage(fd, "Todos os items dropados foram limpos.");
	return 0;
}
...
int atcommand_npctalk(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^,], %99[^\n]", name, mes) < 2) {
		clif_displaymessage(fd, "Por favor, entre com as informações corretas (Como usar: @npctalk <nome do NPC>, <mensagem>).");
		return -1;
	}
...
int atcommand_pettalk(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mes) < 1) {
		clif_displaymessage(fd, "Por favor, entre com uma mensagem (Como usar: @pettalk <mensagem>");
		return -1;
	}
...
int atcommand_summon(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Por favor, entre com o nome de um monstro (Como usar: @summon <nome do monstro> [duração]");
		return -1;
	}
...
int atcommand_adjcmdlvl(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Como usar: @adjcmdlvl <nível> <comando>.");
		return -1;
	}
	...
	if (cmd == NULL)
	{
		clif_displaymessage(fd, "@command não encontrado.");
		return -1;
	}
	else if (newlev > pc_isGM(sd))
	{
		clif_displaymessage(fd, "Você não pode deixar um comando com nível acima do seu.");
		return -1;
	}
	else if (cmd->level > pc_isGM(sd))
	{
		clif_displaymessage(fd, "Você não pode ajustar o nível de um comando que está acima do seu.");
		return -1;
	}
	else
	{
		cmd->level = newlev;
		clif_displaymessage(fd, "@command nível trocado.");
		return 0;
	}
}
...
int atcommand_adjgmlvl(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
    ...
    if (!message || !*message || sscanf(message, "%d %23[^\r\n]", &newlev, user) != 2) {
        clif_displaymessage(fd, "Como usar: @adjgmlvl <nível> <usuário>.");
        return -1;
    }
...
int atcommand_trade(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @trade <jogador>).");
		return -1;
	}
...
int atcommand_setbattleflag(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%127s %127s", flag, value) != 2) {
        	clif_displaymessage(fd, "Como usar: @setbattleflag <flag> <valor>.");
        	return -1;
    	}

	if (battle_set_value(flag, value) == 0)
	{
		clif_displaymessage(fd, "Desconhecida battle_config flag");
		return -1;
	}

	clif_displaymessage(fd, "battle_config configurada como requerido.");
...
int atcommand_unmute(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: @unmute <jogador>).");
		return -1;
	}
	...
	if(!pl_sd->sc.data[SC_NOCHAT]) {
		clif_displaymessage(sd->fd,"O jogador não está bloqueado para falar");
		return -1;
	}

	pl_sd->status.manner = 0;
	status_change_end(&pl_sd->bl,SC_NOCHAT,-1);
	clif_displaymessage(sd->fd,"Jogador permitido a falar");
...
int atcommand_mute(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &manner, atcmd_player_name) < 1) {
		clif_displaymessage(fd, "Como usar: @mute <tempo> <nome do char>.");
		return -1;
	}
...
int atcommand_identify(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (num > 0) {
		...
		clif_displaymessage(fd,"Não há nenhum item para ser identificado.");
	}
	return 0;
}
...
int atcommand_mobinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre um Monstro/ID (Como usar: @mobinfo <nome do monstro/ID>).");
		return -1;
	}
	...
	for (k = 0; k < count; k++) {
		...
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, " Nível:%d  HP:%d  SP:%d  Base EXP:%u  Job EXP:%u", mob->lv, mob->status.max_hp, mob->status.max_sp, mob->base_exp, mob->job_exp);
		clif_displaymessage(fd, atcmd_output);
		...
		sprintf(atcmd_output, " ATK:%d~%d  Range:%d~%d~%d  Size:%s  Raça: %s  Elemento: %s (Lv:%d)",
		...
		if (j == 0)
			clif_displaymessage(fd, "Este monstro não tem drops.");
		else if (j % 3 != 0)
			clif_displaymessage(fd, atcmd_output);
		// mvp
		if (mob->mexp) {
			...
			if (j == 0)
				clif_displaymessage(fd, "Esse monstro não dá nenhum prêmio MVP.");
			else
				clif_displaymessage(fd, atcmd_output);
		}
	}
...
//#define SHOW_MVP
    ...
    if(mob_db(mob_id)->status.mode&MD_BOSS){
        snprintf(atcmd_output, sizeof atcmd_output, "Não é possível monstrar monstros da classe Chefe!");
        clif_displaymessage(fd, atcmd_output);
        return 0;
    }
#endif
...
//                strcpy(mob_name,mob_db(mob_id)->name);    // --en--
    ...
    snprintf(atcmd_output, sizeof atcmd_output, "Procura por mob´s... %s %s",
...
int atcommand_homlevel(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre o valor do nível que deseja ajustar: (Como usar: @homlevel <+/- # de níveis>.");
		return -1;
	}
...
int atcommand_makehomun(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre o ID de um Homunculo: (Como usar: @makehomun <ID do homunculo>.");
		return -1;
	}
	...
	{
		clif_displaymessage(fd, "ID de homunculo inválido.");
		return -1;
	}
...
int atcommand_homfriendly(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre um valor de amizade: (Como usar: @homfriendly <valor [0-1000]>.");
		return -1;
	}
...
int atcommand_homhungry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre um valor para a fome: (Como usar: @homhungry <valor entre [0-100]>.");
		return -1;
	}
...
int atcommand_homtalk(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mes) < 1) {
		clif_displaymessage(fd, "Por favor, entre uma mensagem (Como usar: @homtalk <mensagem>");
		return -1;
	}
...
int atcommand_hominfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	clif_displaymessage(fd, "Stats do homunculo:");
...
int atcommand_homshuffle(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Como usar: @homshuffle <nome do Alquimista>");
		clif_displaymessage(fd, "Use isto para recalcular sua (ou de outro) data de criação do homunculos");
		return -1;
	}
	if (message && *message) {
	...
	clif_displaymessage(sd->fd, "[Status do homunculos alterado]");
...
{
	char *itype[12] = {"Poção/Comida", "BUG!", "Usável", "Etc", "Arma", "Armadura", "Carta", "Ovo", "Acessório de PET", "BUG!", "Flecha"};
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome do item ou ID (Como usar: @ii/@iteminfo <nome do item ou ID>).");
		return -1;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->maxchance == 10000)
			strcpy(atcmd_output, " - Disponível somente em lojas");
		else if (item_data->maxchance)
			sprintf(atcmd_output, " - Chance máxima de drop em monstros: %02.02f%%", (float)item_data->maxchance / 100 );
		else
...
int atcommand_whodrops(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o nome do item ou seu ID (Como usar: @whodrops <nome do item ou ID>).");
		return -1;
	}
	if ((item_array[0] = itemdb_exists(atoi(message))) == NULL)
		count = itemdb_searchname_array(item_array, MAX_SEARCH, message);
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			...
			sprintf(atcmd_output, "- Monstros com a maior chance de drop (somente a maior %d é listada):", MAX_SEARCH);
			clif_displaymessage(fd, atcmd_output);
...
int atcommand_whereis(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com um monstro/ID (Como usar: @whereis<nome do monstro/ID>).");
		return -1;
	}
...
int atcommand_adopt(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^,],%23[^,],%23[^\r\n]", player1, player2, player3) < 3) {
		clif_displaymessage(fd, "Como usar: @adopt <pai>,<mãe>,<filho>.");
		return -1;
	}
	...
	if((pl_sd1=map_nick2sd((char *) player1)) == NULL) {
		sprintf(output, "Não foi possível encontrar o player %s online", player1);
		clif_displaymessage(fd, output);
		return -1;
	}

	if((pl_sd2=map_nick2sd((char *) player2)) == NULL) {
		sprintf(output, "Não foi possível encontrar o player %s online", player2);
		clif_displaymessage(fd, output);
		return -1;
	}
 
	if((pl_sd3=map_nick2sd((char *) player3)) == NULL) {
		sprintf(output, "Não foi possível encontrar o player %s online", player3);
		clif_displaymessage(fd, output);
		return -1;
	}
	...
	clif_displaymessage(fd, "São uma família... deseje-lhes sorte.");
	return 0;
}
...
int atcommand_version(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if ((revision = get_svn_revision()) != 0) {
		sprintf(atcmd_output,"Cronus Versão do SVN r%s",revision);
		clif_displaymessage(fd,atcmd_output);
	} else 
...
int atcommand_mutearea(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, entre com o tempo em minutos (Como usar: @mutearea/@stfu <tempo em minutos>.");
		return -1;
	}
...
int atcommand_rates(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(buf, "Rates do servidor: Base %.1fx / Job %.1fx",
...
int atcommand_me(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%199[^\n]", tempmes) < 0) {
		clif_displaymessage(fd, "Por favor, entre com uma mensagem (Como usar: @me <mensagem>).");
		return -1;
	}
...
int atcommand_monsterignore(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!sd->state.monster_ignore) {
		...
		clif_displaymessage(sd->fd, "Agora você é imune a ataques.");
	} else {
		sd->state.monster_ignore = 0;
		clif_displaymessage(sd->fd, "Voltou ao estado normal.");
	}
...
int atcommand_fakename(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if((!message || !*message) && strlen(sd->fakename) > 1) {
		...
		clif_displaymessage(sd->fd,"Voltou ao nome real.");
		return 0;
	}

	if (!message || !*message || sscanf(message, "%23[^\n]", name) < 1) {
		clif_displaymessage(sd->fd,"Você deve entrar com um nome.");
		return -1;
	}

	if(strlen(name) < 2) {
		clif_displaymessage(sd->fd,"O nome falso deve conter no mínimo 2 letras.");
		return -1;
	}
...
int atcommand_showzeny(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sd->state.showzeny = 1;
	clif_displaymessage(fd, "Zeny ganho será mostrado");
	return 0;
}
...
int atcommand_showdelay(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	if (sd->state.showdelay) {
		...
		clif_displaymessage(fd, "Falhas no skill delay não serão mostradas.");
		return 0;
	}
	
	sd->state.showdelay = 1;
	clif_displaymessage(fd, "Falhas no skill delay serão mostradas.");
	return 0;
}
...
int atcommand_barricade(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	case 2: clif_displaymessage(fd, "Barricade fail. Evento Inválido."); break;
	case 3: clif_displaymessage(fd, "Barricade fail. Evento já existe"); break;
	case 4: clif_displaymessage(fd, "Barricade fail. Problemas com muro"); break;
	}
...
int atcommand_clone(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(sd->fd,"Você deve entrar com um nome ou o id do char.");
		return 0;
	}
...
int atcommand_config_read(const char* cfgName)
{
	...
	if( (fp = fopen(cfgName, "r")) == NULL )
	{
		ShowError("Configuração de AtCommand não encontrado: %s\n", cfgName);
		return 1;
	}
