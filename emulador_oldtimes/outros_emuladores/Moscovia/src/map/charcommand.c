...
int charcommand_jobchange(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &job, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com o nome da classe e o nome do jogador (Como usar: #job/#jobchange <ID da classe/nome> <jogador>).");
		clif_displaymessage(fd, "   0 Aprendiz            7 Cavaleiro              14 Templ·rio           21 Templ·rio (Peco)");
		clif_displaymessage(fd, "   1 Espadachim          8 Sacerdote              15 Monge               22 Casamento");
		clif_displaymessage(fd, "   2 Mago                9 Bruxo                  16 S·bio               23 Super Aprendiz");
		clif_displaymessage(fd, "   3 Arqueiro           10 Ferreiro               17 Arruaceiro          24 Gunslinger");
		clif_displaymessage(fd, "   4 NoviÁo             11 CaÁador                18 Alquimista          25 Ninja");
		clif_displaymessage(fd, "   5 Mercador           12 Mercen·rio             19 Bardo               26 Natal");
		...
		clif_displaymessage(fd, "4005 NoviÁo T.        4012 Atirador Elite (N„o utilize o de)    4019 Criador");
		clif_displaymessage(fd, "4006 Mercador T.      4013 Algoz                4020 Menestrel");
		clif_displaymessage(fd, "4007 Gatuno T.        4014 Peco Lord Knight     4021 Cigana");
		clif_displaymessage(fd, "4023 BebÍ Aprendiz    4030 BebÍ Cavaleiro       4037 BebÍ Templ·rio    4044 BebÍ Templ·rio (Peco) ");
		clif_displaymessage(fd, "4024 BebÍ Espadachim  4031 BebÍ Sacerdote       4038 BebÍ Monge        4045 Super BebÍ");
		clif_displaymessage(fd, "4025 BebÍ Mago        4032 BebÍ Bruxo           4039 BebÍ S·bio        4046 Taekwon");
		clif_displaymessage(fd, "4026 BebÍ Arqueiro    4033 BebÍ Ferreiro        4040 BebÍ Arruaceiro   4047 Mestre Taekwon");
		clif_displaymessage(fd, "4027 Baby Acolyte     4034 BebÍ CaÁador         4041 BebÍ Alquimista   4048 N/A");
		clif_displaymessage(fd, "4028 BebÍ Mercador    4035 BebÍ Mercen·rio      4042 BebÍ Bardo        4049 Espiritualista");
		clif_displaymessage(fd, "4029 BebÍ Gatuno      4036 BebÍ Cavaleiro Peco  4043 BebÍ DanÁarina");
		return -1;
	}
...
int charcommand_petrename(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: #petrename <jogador>).");
		return -1;
	}
...
int charcommand_petfriendly(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message,"%d %23s",&friendly,character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com um valor v·lido (Como usar: #petfriendly <0-1000> <jogador>).");
		return -1;
	}
...
int charcommand_stats(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: #stats <jogador>).");
		return -1;
	}
	...
	sprintf(job_jobname, "Classe - %s %s", job_name(pl_sd->status.class_), "(nÌvel %d)");
...
int charcommand_reset(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: #reset <jogador>).");
		return -1;
	}
...
int charcommand_option(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message ||
		...
		clif_displaymessage(fd, "Por favor, entre com opÁıes v·lidas e o nome de um jogador (Como usar: #option <param1> <param2> <param3> <jogador>).");
		return -1;
	}
...
int charcommand_save(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%15s %d %d %23[^\n]", map_name, &x, &y, character) < 4 || x < 0 || y < 0) {
		clif_displaymessage(fd, "Por favor, entre um ponto de retorno v·lido e o nome de um jogador (Como usar: #save <mapa> <x> <y> <jogador>).");
		return -1;
	}
	...
	if (m>=0 && map[m].flag.nosave && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
		clif_displaymessage(fd, "VocÍ n„o est· autorizado a salvar seu ponto de retorno neste mapa.");
		return -1;
	}
...
int charcommand_spiritball(const int fd, struct map_session_data* sd,const char* command, const char* message)
{
	...
	if(!message || !*message || sscanf(message, "%d %23[^\n]", &spirit, character) < 2 || spirit < 0 || spirit > 1000) {
		clif_displaymessage(fd, "Como usar: @spiritball <quantidade: 0-1000> <jogador>.");
		return -1;
	}
...
int charcommand_itemlist(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: #itemlist <jogador>).");
		return -1;
	}
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		...
		if (pl_sd->status.inventory[i].nameid > 0 && (item_data = itemdb_exists(i_item->nameid)) != NULL) {
			...
			count++;
			if (count == 1) {
				sprintf(output, "------ Lista de items de '%s' ------", pl_sd->status.name);
				clif_displaymessage(fd, output);
			}
			if ((equip = i_item->equip)) {
				...
				if (equip & EQP_ACC_L)
					strcat(equipstr, "acessÛrio esquerdo, ");
				if (equip & EQP_ARMOR)
					strcat(equipstr, "corpo/armadura, ");
				if ((equip & EQP_ARMS) == EQP_HAND_R)
					strcat(equipstr, "m„o direita, ");
				if ((equip & EQP_ARMS) == EQP_HAND_L)
					strcat(equipstr, "m„o esquerda,");
				if ((equip & EQP_ARMS) == EQP_ARMS)
					strcat(equipstr, "duas m„os, ");
				if (equip & EQP_SHOES)
					strcat(equipstr, "pÈs, ");
				if (equip & EQP_ACC_R)
					strcat(equipstr, "acessÛrio direito, ");
				if ((equip & EQP_HELM) == EQP_HEAD_LOW)
					strcat(equipstr, "cabeÁa baixo, ");
				if ((equip & EQP_HELM) == EQP_HEAD_TOP)
					strcat(equipstr, "cabeÁa topo, ");
				if ((equip & EQP_HELM) == (EQP_HEAD_LOW|EQP_HEAD_TOP))
					strcat(equipstr, "cabeÁa baixo/topo, ");
				if ((equip & EQP_HELM) == EQP_HEAD_MID)
					strcat(equipstr, "cabeÁa meio, ");
				if ((equip & EQP_HELM) == (EQP_HEAD_LOW|EQP_HEAD_MID))
					strcat(equipstr, "cabeÁa baixo/meio, ");
				if ((equip & EQP_HELM) == EQP_HELM)
					strcat(equipstr, "cabeÁa baixo/meio/topo, ");
				// remove final ', '
			...
			if(i_item->card[0]==CARD0_PET) { //pet eggs
				if (i_item->card[3])
					sprintf(outputtmp, " -> (ovo de pet, ID do pet: %u, nomeado)", (unsigned int)MakeDWord(i_item->card[1], i_item->card[2]));
				else
					sprintf(outputtmp, " -> (ovo de pet, ID do pet: %u, n„o nomeado)", (unsigned int)MakeDWord(i_item->card[1], i_item->card[2]));
				strcat(output, outputtmp);
			} else
			if(i_item->card[0]==CARD0_FORGE) { //forged items.
				sprintf(outputtmp, " -> (Item quebrado, ID dro criador: %u, star crumbs %d, element %d)", (unsigned int)MakeDWord(i_item->card[2], i_item->card[3]), i_item->card[1]>>8, i_item->card[1]&0x0f);
			} else
	...
	if (count == 0)
	{
		clif_displaymessage(fd, "N„o foram encontrados items neste jogador.");
		return -1;
	}

	sprintf(output, "%d item(s) encontrados em %d tipo(s) de items.", counter, count);
	clif_displaymessage(fd, output);
	return 0;
}
...
int charcommand_effect(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &type, character) != 2) {
		clif_displaymessage(fd, "Como usar: #effect <tipo+> <alvo>.");
		return -1;
	}
...
int charcommand_storagelist(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome de um jogador (Como usar: #itemlist <nome do jogador>).");
		return -1;
	}
	...
	if((stor = account2storage2(pl_sd->status.account_id)) != NULL) {
		...
		for (i = 0; i < MAX_STORAGE; i++) {
			if (stor->storage_[i].nameid > 0 && (item_data = itemdb_search(stor->storage_[i].nameid)) != NULL) {
				...
				count++;
				if (count == 1) {
					sprintf(output, "------ Lista de items do armazÈm de '%s' ------", pl_sd->status.name);
					clif_displaymessage(fd, output);
				}
				...
				for (j = 0; j < item_data->slot; j++) {
					if (stor->storage_[i].card[j]) {
						if ((item_temp = itemdb_search(stor->storage_[i].card[j])) != NULL) {
							if (output[0] == '\0')
								sprintf(outputtmp, " -> (carta(s): #%d %s (%s), ", ++counter2, item_temp->name, item_temp->jname);
							else
								sprintf(outputtmp, "#%d %s (%s), ", ++counter2, item_temp->name, item_temp->jname);
							strcat(output, outputtmp);
						}
		...
		if (count == 0)
			clif_displaymessage(fd, "N„o foram encontrados items no armazÈm deste jogador.");
		else {
			sprintf(output, "%d item(s) encontrados em %d tipos de item.", counter, count);
			clif_displaymessage(fd, output);
		}
	} else {
		clif_displaymessage(fd, "Este jogador n„o tem armazÈm.");
		return 0;
	}
...
/*==========================================
 * #item command (Como usar: #item <name/id_of_item> <quantity> <player>)
 * by MC Cameri
 *------------------------------------------*/
int charcommand_item(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com o nome do item/ID (Como usar: #item <nome do item/ID> <quantidade> <nome do char>).");
		return -1;
	}
	...
	else if (strcmpi(character,"all")==0 || strcmpi(character,"todos")==0)
	{
		...
		snprintf(tmp_cmdoutput, sizeof(tmp_cmdoutput), "todos receberam %s %d.",item_name,number);
...
int charcommand_warp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%15s %d %d %23[^\n]", map_name, &x, &y, character) < 4) {
		clif_displaymessage(fd, "Como usar: #warp/#rura/#rura+ <nome do mapa> <x> <y> <nome do jogador>");
		return -1;
	}
	...
	if (map[m].flag.nowarpto && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
		clif_displaymessage(fd, "VocÍ n„o est· autorizado a transportar uma pessoa a este mapa.");
		return -1;
	}
	if (pl_sd->bl.m >= 0 && map[pl_sd->bl.m].flag.nowarp && battle_config.any_warp_GM_min_level > pc_isGM(sd)) {
		clif_displaymessage(fd, "VocÍ n„o est· autorizado a teletransportar o jogador de seu mapa atual.");
		return -1;
	}
...
int charcommand_zeny(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &zeny, character) < 2 || zeny == 0) {
		clif_displaymessage(fd, "Por favor, entre um n˙mero e o nome do jogador (Como usar: #zeny <quantidade> <nome do jogador>).");
		return -1;
	}
...
int charcommand_fakename(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23s %23[^\n]", char_name, name) < 1) {
		clif_displaymessage(sd->fd,"Como usar: #fakename <nome do char> <nome falso>.");
		clif_displaymessage(sd->fd,"Ou: #fakename <nome do char> para desabilitar.");
		return 0;
	}

	if(!(pl_sd = map_nick2sd(char_name))) {
		clif_displaymessage(sd->fd,"Jogador n„o encontrado.");
		return -1;
	}
	...
	if(!name[0]) {
		if(strlen(pl_sd->fakename) > 1) {
			...
			clif_displaymessage(sd->fd,"Retornou ao nome real.");
		} else {
			clif_displaymessage(sd->fd,"O Jogador n„o est· usando um nome falso.");
		}
		return 0;
	}
...
int charcommand_baselevel(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &level, player) < 2 || level == 0) {
		clif_displaymessage(fd, "Por favor, entre um ajuste de nÌvel e o nome do jogador (Como usar: #baselvl <#> <nome do jogador>).");
		return -1;
	}
...
int charcommand_joblevel(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &level, player) < 2 || level == 0) {
		clif_displaymessage(fd, "Por favor, entre um ajuste de nÌvel e o nome do jogador (Como usar: #joblvl <#> <nome do jogador>).");
		return -1;
	}
...
int charcommand_questskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &skill_id, player) < 2 || skill_id < 0) {
		clif_displaymessage(fd, "Por favor, enter a quest skill number and a player name (Como usar: #questskill <#:0+> <nickname>).");
		return -1;
	}
...
int charcommand_lostskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &skill_id, player) < 2 || skill_id < 0) {
		clif_displaymessage(fd, "Por favor, entre com o n˙mero da skill e o nome do jogador (Como usar: @charlostskill <#:0+> <nome do jogador>).");
		return -1;
	}
...
int charcommand_skreset(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", player) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: @charskreset <nome do jogador>).");
		return -1;
	}
...
int charcommand_streset(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", player) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: @charstreset <nome do jogador>).");
		return -1;
	}
...
int charcommand_model(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d %23[^\n]", &hair_style, &hair_color, &cloth_color, player) < 4 || hair_style < 0 || hair_color < 0 || cloth_color < 0) {
		sprintf(tmp_cmdoutput, "Por favor, entre com um modelo v·lido e o nome do jogador (Como usar: @charmodel <ID do cabelo: %d-%d> <Cor do cabelo: %d-%d> <Cor da roupa: %d-%d> <nome>).",
...
int charcommand_skpoint(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &point, player) < 2 || point == 0) {
		clif_displaymessage(fd, "Por favor, entre com um n˙mero e o nome do jogador (Como usar: @charskpoint <quantidade> <nome do jogador>).");
		return -1;
	}
...
int charcommand_stpoint(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &point, player) < 2 || point == 0) {
		clif_displaymessage(fd, "Por favor, entre com um n˙mero e o nome do jogador (Como usar: @charstpoint <quantidade> <nome do jogador>).");
		return -1;
	}
...
/*==========================================
 * charchangesex command (Como usar: charchangesex <player_name>)
 *------------------------------------------*/
int charcommand_changesex(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", player) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: @charchangesex <nome do jogador>).");
		return -1;
	}
...
int charcommand_feelreset(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #feelreset <nome do jogador>).");
		return -1;
	}
...
int charcommand_load(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #load <jogador>).");
		return -1;
	}
	...
	if (m >= 0 && map[m].flag.nowarpto && battle_config.any_warp_GM_min_level > pc_isGM(pl_sd)) {
		clif_displaymessage(fd, "N„o est· autorizado a teletransportar para este mapa.");
		return -1;
	}
	if (pl_sd->bl.m >= 0 && map[pl_sd->bl.m].flag.nowarp && battle_config.any_warp_GM_min_level > pc_isGM(pl_sd)) {
		clif_displaymessage(fd, "N„o est· autorizado a teletransportar do mapa atual.");
		return -1;
	}
...
int charcommand_speed(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &speed, character) < 2) {
		sprintf(output, "Por favor, entre com valores v·lidos (Como usar: #speed <%d-%d> <jogador>).", MIN_WALK_SPEED, MAX_WALK_SPEED);
		clif_displaymessage(fd, output);
		return -1;
	}
...
int charcommand_storage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #storage <jogador>).");
		return -1;
	}
	...
	if (storage_storageopen(pl_sd) == 1)
  	{	//Already open.
		clif_displaymessage(fd, "ArmazÈm do jogador j· est· aberto.");
		return -1;
	}
	
	clif_displaymessage(pl_sd->fd, "ArmazÈm aberto.");
	if (pl_sd->fd != fd)
		clif_displaymessage(fd, "ArmazÈm do jogador aberto.");
...
int charcommand_guildstorage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #gstorage <jogador>).");
		return -1;
	}
	...
	if (pl_sd->status.guild_id > 0) {
		if (pl_sd->state.storage_flag) {
			clif_displaymessage(fd, "ArmazÈm da guild j· est· em uso.");
			return -1;
		}
		...
		clif_displaymessage(fd, "O jogador alvo n„o pertence a nenhuma guild.");
		return -1;
	}

	clif_displaymessage(pl_sd->fd, "ArmazÈm da guild aberto.");
	if (pl_sd->fd != fd)
		clif_displaymessage(fd, "ArmazÈm da guild do jogador aberto.");
...
int charcommand_hide(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #hide <jogador>).");
		return -1;
	}
...
int charcommand_alive(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #alive <jogador>).");
		return -1;
	}
	...
	{
		clif_displaymessage(fd, "O jogador alvo n„o est· morto.");
		return -1;
	}
...
int charcommand_heal(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %23[^\n]", &hp, &sp, character) < 3) {
		clif_displaymessage(fd, "Por favor, entre com valores v·lidos (Como usar: #heal <hp> <sp> <jogador>).");
		return -1;
	}
...
int charcommand_item2(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com todas as informaÁıes (Como usar: @item2 <nome do item/ID> <quantidade>");
		clif_displaymessage(fd, "  <Identificado?[1=S|0=N]> <refine> <atributo> <Carta1> <Carta2> <Carta3> <Carta4> <jogador>).");
		return -1;
	}
...
int charcommand_itemreset(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #itemreset <jogador>).");
		return -1;
	}
...
int charcommand_refine(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %23[^\n]", &position, &refine, character) < 3) {
		clif_displaymessage(fd, "Por favor, entre com uma posiÁ„o, um valor e o nome do jogador (Como usar: #refine <posiÁ„o do equip> <+/- quantidade> <nome do jogador>).");
		sprintf(output, "%d: AcessÛrio Esquerdo", EQI_ACC_L);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: AcessÛrio Direito", EQI_ACC_R);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: CalÁado", EQI_SHOES);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: Capa", EQI_GARMENT);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: CabeÁa Baixo", EQI_HEAD_LOW);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: CabeÁa Meio", EQI_HEAD_MID);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: CabeÁa Topo", EQI_HEAD_TOP);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: Armadura", EQI_ARMOR);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: M„o Esquerda", EQI_HAND_L);
		clif_displaymessage(fd, output);
		sprintf(output, "%d: M„o Direita", EQI_HAND_R);
		clif_displaymessage(fd, output);
		return -1;
	}
...
int charcommand_produce(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com no mÌnimo um nome de item/ID e o nome do jogador.");
		clif_displaymessage(fd, "(Como usar: #produce <nome do equipamento/ID> <elemento> <# de very's> <jogador>).");
		return -1;
	}
...
int charcommand_param(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %s", &value, character) < 2 || value == 0) {
		sprintf(output, "Por favor, entre com um valor v·lido (Como usar: #str,#agi,#vit,#int,#dex,#luk <+/- quantidade> <jogador>).");
		clif_displaymessage(fd, output);
		return -1;
	}
...
int charcommand_guildlevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %s", &level, character) < 2 || level == 0) {
		clif_displaymessage(fd, "Por favor, entre com um nÌvel v·lido e o nome do jogador (Como usar: #guildlvup/@guildlvlup <# de nÌveis> <jogador>).");
		return -1;
	}
	...
	if (pl_sd->status.guild_id <= 0 || (guild_info = guild_search(pl_sd->status.guild_id)) == NULL) {
		clif_displaymessage(fd, "O jogador alvo n„o pertence a uma guild."); // You're not in a guild.
		return -1;
	}
...
int charcommand_hatch(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #hatch <jogador>).");
		return -1;
	}
	...
	if (pl_sd->status.pet_id <= 0)
		...
		clif_displaymessage(fd, "O jogador alvo j· tem um pet."); // You already have a pet.
		return -1;
	}
...
int charcommand_pethungry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &hungry, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com um n˙mero v·lido e o nome do jogador (Como usar: #pethungry <0-100> <jogador>).");
		return -1;
	}
	...
	if (!pl_sd->status.pet_id || !pd) {
		clif_displaymessage(fd, "O jogador alvo n„o tem um pet."); // Sorry, but you have no pet.
		return -1;
	}
...
int charcommand_allskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #allskill <jogador>).");
		return -1;
	}
	...
	if (pl_sd->fd != fd)
		clif_displaymessage(fd, "O jogador recebeu todas as habilidades para seu/sua classe.");
...
int charcommand_dye(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &cloth_color, character) < 2) {
		sprintf(output, "Por favor, entre com a cor da roupa e o nome do jogador (Como usar: #dye/@ccolor <Cor da roupa: %d-%d> <nome do jogador>).", MIN_CLOTH_COLOR, MAX_CLOTH_COLOR);
		clif_displaymessage(fd,output);
		return -1;
	}
...
int charcommand_hair_style(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &hair_style, character) < 2) {
		sprintf(output, "Por favor, entre com um estilo de cabelo e o nome do jogador (Como usar: #hairstyle/#hstyle <ID do cabelo: %d-%d> <jogador>.", MIN_HAIR_STYLE, MAX_HAIR_STYLE);
		clif_displaymessage(fd, output);
		return -1;
	}
...
int charcommand_hair_color(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &hair_color, character) < 2) {
		sprintf(output, "Por favor, entre com a cor de um cabelo (Como usar: #haircolor/#hcolor <Cor do cabelo: %d-%d> <nome do jogador>).", MIN_HAIR_COLOR, MAX_HAIR_COLOR);
		clif_displaymessage(fd, output);
		return -1;
	}
...
int charcommand_allstats(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #statall/#statsall/#allstat/#allstats <nome do jogador>).");
		return -1;
	}
	...
	if (count > 0) // if at least 1 stat modified
	{
		...
		if (fd != pl_sd->fd)
			clif_displaymessage(fd, "Status do jogador alterados."); // All stats changed!
	}
	else {
		if (value < 0)
...
int charcommand_mount_peco(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #mount/#mountpeco <jogador>).");
		return -1;
	}
	...
	{ // if actually no peco
		...
		if (pl_sd->fd != fd)
			clif_displaymessage(fd, "O jogador montou no PecoPeco.");
	...
	{	//Dismount
		...
		if (pl_sd->fd != fd)
			clif_displaymessage(fd, "O jogador desmontou do PecoPeco.");
	}
...
int charcommand_delitem(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "Por favor, entre com o nome do item/ID, uma quantidade e o nome do jogador (Como usar: #delitem <nome do item/ID> <quantidade> <jogador>).");
		return -1;
	}
...
{  
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #jailtime <nome do jogador>).");
		return -1;
	}
	...
	if (!pl_sd->sc.data[SC_JAILED]) {
		clif_displaymessage(fd, "Este jogador n„o est· na cadeia.");
		return -1;
	}

	if (pl_sd->sc.data[SC_JAILED]->val1 == INT_MAX) {
		clif_displaymessage(fd, "Este jogador foi preso por tempo infinito.");
		return 0;
	}

	if (pl_sd->sc.data[SC_JAILED]->val1 <= 0) { // Was not jailed with @jailfor (maybe @jail? or warped there? or got recalled?)
		clif_displaymessage(fd, "Este jogador foi preso por um tempo desconhecido.");
		return -1;
	}
	//Get remaining jail time
	get_jail_time(pl_sd->sc.data[SC_JAILED]->val1,&year,&month,&day,&hour,&minute);
	sprintf(output,msg_txt(402),"Este jogador ir· permanecer",year,month,day,hour,minute); 
	clif_displaymessage(fd, output);
...
int charcommand_disguise(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%s %23[^\n]", mob_name, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre um o nome de um monstro/NPC ou ID e o nome de um jogador (Como usar: #disguise <nome do monstro/NPC ou ID> <nome do jogador>).");
		return -1;
	}
...
int charcommand_undisguise(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #undisguise <nome do jogador>).");
		return -1;
	}
...
int charcommand_cart_list(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #cartlist <nome do jogador>).");
		return -1;
	}
	...
	for (i = 0; i < MAX_CART; i++) {
		if (pl_sd->status.cart[i].nameid > 0 && (item_data = itemdb_search(pl_sd->status.cart[i].nameid)) != NULL) {
			...
			count++;
			if (count == 1) {
				sprintf(output, "------ Lista dos items do carrinho de '%s' ------", pl_sd->status.name);
				clif_displaymessage(fd, output);
			}
	...
	if (count == 0)
		clif_displaymessage(fd, "Nenhum item encontrado no carrinho deste jogador.");
	else {
		sprintf(output, "%d item(s) encontrados em %d diferentes tipos de item.", counter, count);
		clif_displaymessage(fd, output);
	}
...
int charcommand_killer(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #killer <jogador>).");
		return -1;
	}
	...
	if(pl_sd->state.killer)
	{
		...
		if (pl_sd->fd != fd)
			clif_displaymessage(fd, "O jogador alvo agora pode matar a todos.");
	}
	else
	{
...
int charcommand_killable(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #killable <jogador>).");
		return -1;
	}
...
int charcommand_dropall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	clif_displaymessage(pl_sd->fd, "Todos os items foram derrubados.");
	clif_displaymessage(fd, "Est· feito.");
...
int charcommand_storeall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (pl_sd->state.storage_flag != 1)
  	{	//Open storage.
		...
		case 2: //Try again
			clif_displaymessage(fd, "A janela do armazÈm do jogador teve de ser aberta...");
			clif_displaymessage(fd, "execute o comando novamente..");
			return 0;
		case 1: //Failure
			clif_displaymessage(fd, "O jogador n„o pode usar o armazÈm no momento.");
			return 1;
		}
	}
	...
	clif_displaymessage(pl_sd->fd, "Todos os seus bens foram movidos para seu armazÈm para que n„o hajam riscos.");
	clif_displaymessage(pl_sd->fd, "v· para a kafra mais prÛxima e retire-os..");
	clif_displaymessage(pl_sd->fd, "   -- a gerÍncia");
...
int charcommand_refresh(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #refresh <nome do jogador>).");
		return -1;
	}
...
int charcommand_exp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #exp <nome do jogador>).");
		return -1;
	}
	...
	sprintf(output, "NÌvel de Base: %d (%.3f%%) | NÌvel de Classe: %d (%.3f%%)", pl_sd->status.base_level, nextb, pl_sd->status.job_level, nextj);
	clif_displaymessage(fd, output);
	return 0;
}
...
int charcommand_monsterignore(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #monsterignore/#battleignore <nome do jogador>).");
		return -1;
	}
	...
	if (!pl_sd->state.monster_ignore) {
		...
		clif_displaymessage(pl_sd->fd, "VocÍ est· agora imune a ataques.");
		if (fd != pl_sd->fd)
			clif_displaymessage(pl_sd->fd, "O jogador alvo agora est· imune a ataques.");
	} else {
		sd->state.monster_ignore = 0;
		clif_displaymessage(sd->fd, "VocÍ n„o est· mais imune a ataques.");
		if (fd != pl_sd->fd)
			clif_displaymessage(pl_sd->fd, "O jogador alvo n„o est· mais imune a ataques.");
...
int charcommand_size(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &size, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #size <0-2> <nome do jogador>).");
		return -1;
	}
	...
	if(size==1) {
		...
		clif_displaymessage(fd, "Tamanho mudado."); 
		if (fd != pl_sd->fd)
			clif_displaymessage(pl_sd->fd, "Tamanho mudado."); 
	} else if(size==2) {
		pl_sd->state.size=2;
	...
	{
		clif_displaymessage(fd, "Tamanho original restaurado."); 
		if (fd != pl_sd->fd)
			clif_displaymessage(pl_sd->fd, "Tamanho original restaurado."); 
	}
...
int charcommand_homlevel(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &level, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador e o nÌvel (Como usar: #homlevel <# de nÌveis> <jogador>).");
		return -1;
	}
	...
	{
		clif_displaymessage(fd, "O jogador alvo n„o tem um Homunculo."); 
		return 1;
	}
	...
	clif_displaymessage(pl_sd->fd, "NÌvel do Homunculo mudado."); 
	if (pl_sd->fd != fd)
		clif_displaymessage(fd, "NÌvel do Homunculo do jogador foi mudado.");
...
int charcommand_homevolution(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #homevolution <nome do jogador>).");
		return -1;
	}
	...
	if ( !merc_hom_evolution(pl_sd->hd) ) {
		clif_displaymessage(fd, "O homunculo alvo n„o pode evoluir."); 
		return -1;
	}

	clif_displaymessage(pl_sd->fd, "EvoluÁ„o do homunculo iniciada."); 
	if (pl_sd->fd != fd)
		clif_displaymessage(fd, "EvoluÁ„o do homunculo iniciada."); 
	return 0;
}
...
int charcommand_makehomun(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &homunid, character) < 2) {
		sprintf(output, "Por favor, entre com o nome do jogador (Como usar: #makehomun <ID do homunculo: %d-%d> <nome do jogador>).", HM_CLASS_BASE, HM_CLASS_BASE + MAX_HOMUNCULUS_CLASS - 1);
		clif_displaymessage(fd, output);
		return -1;
	}
	...
	{
		clif_displaymessage(fd, "ID do homunculo inv·lido."); 
		return -1;
	}
	...
	{
		clif_displaymessage(fd, "O jogador alvo j· tem um homunculo.");
	}
	return 0;
}
...
int charcommand_homfriendly(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &friendly, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador e o valor de amizade (Como usar: #homfriendly <0-1000> <nome do jogador>).");
		return -1;
	}
	...
	if (merc_is_hom_active(pl_sd->hd)) {
		if (friendly > 0 && friendly <= 1000) {
			...
			clif_displaymessage(pl_sd->fd, "Valor de amizade do homunculos mudado."); 
			if (fd != pl_sd->fd)
				clif_displaymessage(fd, "Valor de amizade do homunculos mudado."); 
		} else {
			clif_displaymessage(fd, "Valor de amizade inv·lido."); 
		}
	}
	else
	{
		clif_displaymessage(fd, "O homunculo do jogador alvo n„o existe."); 
		return -1;
	}
...
int charcommand_homhungry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %23[^\n]", &hungry, character) < 2) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #homhungry <0-100> <nome do jogador>).");
		return -1;
	}
	...
	if (pl_sd->status.hom_id > 0 && pl_sd->hd) {
		...
		if (hungry >= 0 && hungry <= 100) {
			...
			clif_displaymessage(pl_sd->fd, "Fome do homunculo ajustada."); 
			if (fd != pl_sd->fd)
				clif_displaymessage(fd, "Valor de fome do homunculo ajustado."); 
		} else {
			clif_displaymessage(fd, "Valor de fome incorreto."); 
			return -1;
		}
	}
	else
...
int charcommand_hominfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", character) < 1) {
		clif_displaymessage(fd, "Por favor, entre com o nome do jogador (Como usar: #hominfo <jogador>).");
		return -1;
	}
	...
	snprintf(output, sizeof(output), "Fome : %d - IntimaÁ„o : %u", hd->homunculus.hunger, hd->homunculus.intimacy/100);
	clif_displaymessage(fd, output);

	snprintf(output, sizeof(output), "Status: Str %d / Agi %d / Vit %d / Int %d / Dex %d / Luk %d", status->str, status->agi, status->vit, status->int_, status->dex, status->luk);
	clif_displaymessage(fd, output);
...
int charcommand_config_read(const char* cfgName)
{
	...
	{
		ShowError("Arquivo de configuraÁ„o 'CharCommand' n„o encontrado: %s\n", cfgName);
		return 1;
	}
	...
	while( fgets(line, sizeof(line), fp) )
	{
		...
		if( strcmpi(w1, "command_symbol") == 0 &&
			...
			ShowWarning("ConfiguraÁ„o desconhecida '%s' no arquivo %s\n", w1, cfgName);
	}
	fclose(fp);
