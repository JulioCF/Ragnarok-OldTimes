 ...
 * 定数データベースの読み込み
 *------------------------------------------*/
static void read_constdb(const char *constFolder,const char *constFile)
{
	FILE *fp;
	char line[1024],name[1024],val[1024];
	int n,type;
	sprintf(line, "%s%s",constFolder,constFile);
//		sprintf(line, , "conf");
	fp=fopen(line, "r");
	if(fp==NULL){
...
struct script_code* parse_script(const char *src,const char *file,int line,int options)
{
	...
	if(first){
		add_buildin_func();
		read_constdb(db_path,"/const.txt");
		read_constdb("conf","/npc_athena.conf");
	}
	first=0;
