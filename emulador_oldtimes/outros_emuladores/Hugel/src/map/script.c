...
static void read_npcconf(void)
{
	FILE *fp;
	char line[1024],name[1024],val[1024];
	int n,type;

	sprintf(line, "%s/npc_athena.conf", "conf");
	fp=fopen(line, "r");
	if(fp==NULL){
		ShowError("can't read %s\n", line);
		return ;
...
BUILDIN_FUNC(show); //support Multi-Show [Mehah]
BUILDIN_FUNC(goto);
...
struct script_function buildin_func[] = {
	// NPC interaction
	BUILDIN_DEF(show,"s*"), //support Multi-Show [Mehah]
	BUILDIN_DEF(next,""),
	BUILDIN_DEF(close,""),
	BUILDIN_DEF(close2,""),
	BUILDIN_DEF(menu,"sl*"),
	//BUILDIN_DEF(menu2,"sl*"), // [Mehah]
	BUILDIN_DEF(select,"s*"), //for future jA script compatibility
	BUILDIN_DEF(select2,"s*"),
